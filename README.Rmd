...
6. for numeric or dates, defaults limits to the min/max of x and y breaks and zero expanding
...
6. For numeric or date variables, {ggblanket} defaults limits to the min/max of x and y breaks with zero expanding.

Benefits of this approach is that: 

* it can look visually very nice with gridlines in both directions. 
* it is simple to change to the min and max of geoms on the scale by using `*_limits = c(NA, NA)` (or `c(lubridate::NA_Date_, lubridate::NA_Date_)`).

However, there is something to be aware of for polygons on a numeric or date x axis. This is that the limits may result in the first or last polygon being lost from the plot, if the limits are equal to the first of last value of the x variable. One way to deal with this is to use `x_limits = c(NA, NA)` or `c(lubridate::NA_Date_, lubridate::NA_Date_)`. This is demonstrated below, where if `x_limits = c(1980, 2020)` was used, the first and last column would have been lost. Instead data is filtered, and then x_limits = c(NA, NA) is added, and everything is fine.
