...
8. outputs a {ggplot2} object, so extra `ggplot2::geom_*` layers can be added
...
5. {ggblanket} provides prefixed arguments for scale and title adjustment.
...
6. For numeric or date variables, {ggblanket} defaults limits to the min/max of x and y breaks and zero expanding.
...
8. {ggblanket} outputs a {ggplot2} object, so extra `ggplot2::geom_*` layers can be added.
