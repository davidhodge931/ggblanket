...
#'     x_title = NULL,
#'     x_trans = scales::transform_identity(),
#'     y_breaks = NULL,
#'     y_expand = NULL,
#'     y_gridlines = NULL,
#'     y_include = NULL,
...
#'     y_title = NULL,
#'     y_trans = scales::transform_identity(),
#'     col_breaks = NULL,
...
#'   if (rlang::quo_is_null(x) & !rlang::quo_is_null(y)) flipped <- TRUE
#'   else if (!rlang::quo_is_null(x) & rlang::quo_is_null(y)) flipped <- FALSE
#'   else if (!rlang::quo_is_null(x) & !rlang::quo_is_null(y)) {
#'     flipped <- all(
#'       inherits(rlang::eval_tidy(y, data),
#'                what = c("logical", "factor", "character")),
#'       inherits(rlang::eval_tidy(x, data),
#'                what = c("numeric", "double", "integer", "date", "datetime", "hms", "NULL"))
#'     )
#'   } else flipped <- FALSE
#'
#'   data <- data %>%
...
#'     # if (stat != "sf") {
#'     if (x_numeric) {
#'       if (any(x_trans$name %in% "reverse") & !rlang::is_null(x_limits)) {
#'         plot <- plot +
...
#'
#'     if (y_numeric) {
#'       if (any(y_trans$name %in% "reverse") & !rlang::is_null(y_limits)) {
#'         plot <- plot +
...
#'   if (stringr::str_detect(class(stat)[1], "Sf")) {
#'     # if (stat == "sf") {
#'     if (rlang::quo_is_null(col)) col_scale <- "none"
...
#'     plot <- plot2
#'   }
#'   else {   #correct for plots where col is null, but there is a colour scale
...
#'     } else col_scale <- "none"
#'
#'     if (rlang::quo_is_null(col) & col_scale %in% c("continuous", "discrete")) {
#'       plot <- plot +
#'         ggplot2::layer(
#'           geom = geom,
#'           stat = stat,
...
#'       if (inherits(rlang::eval_tidy(col, data), what = "hms")) col_trans <- scales::transform_hms()
#'       else if (inherits(rlang::eval_tidy(col, data), what = "date")) col_trans <- scales::transform_date()
#'       else col_trans <- scales::transform_identity()
#'     }
#'
#'     if (rlang::is_null(col_breaks)) {
#'       if (!any(col_trans$name %in% c("identity", "reverse"))) col_breaks <- ggplot2::waiver()
#'       else col_breaks <- scales::breaks_pretty(n = 5)
#'     }
#'
#'     if (rlang::is_null(col_labels)) {
#'       if (col_trans$name == "date") col_labels <- scales::label_date(format = c("%Y", "%b", "%e"))
#'       else if (col_trans$name == "hms") col_labels <- scales::label_time(format = "%H:%M")
#'       else if (!any(col_trans$name %in% c("identity", "reverse"))) col_labels <- ggplot2::waiver()
...
#'           }
#'           if (rlang::is_null(x_include)) {
#'             if (any(x_trans$name %in% "reverse")) {
...
#'         if (any(x_trans$name %in% "reverse")) x_range <- sort(x_range, decreasing = TRUE)
#'
...
#'             else if (any(!x_trans$name %in% c("identity", "reverse"))) x_limits <- x_range
...
#'             if (!flipped) x_limits <- x_range
#'             else {
#'               if (any(!x_trans$name %in% c("identity", "reverse"))) {
#'                 x_limits <- x_range
#'               } else {
...
#'             }
#'           }
#'           if (any(x_trans$name %in% "reverse")) x_limits <- sort(x_limits, decreasing = TRUE)
#'         }
...
#'           }
#'           if (rlang::is_null(y_include)) {
#'             if (any(y_trans$name %in% "reverse")) {
...
#'         if (any(y_trans$name %in% "reverse")) y_range <- sort(y_range, decreasing = TRUE)
#'
...
#'             else if (any(!y_trans$name %in% c("identity", "reverse"))) y_limits <- y_range
...
#'             if (flipped) y_limits <- y_range
#'             else {
#'               if (any(!y_trans$name %in% c("identity", "reverse"))) {
#'                 y_limits <- y_range
#'               } else {
...
#'             }
#'           }
#'           if (any(y_trans$name %in% "reverse")) y_limits <- sort(y_limits, decreasing = TRUE)
#'         }
