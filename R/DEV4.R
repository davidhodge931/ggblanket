...
gg_layer <- function(
  ...
  # if (class(rlang::eval_tidy(col, data)) == "logical") {
  #   if (
  ...
  if (flipped) {
    data <- data |>
      dplyr::mutate(dplyr::across(!!col & tidyselect::where(is.character),
                                  function(x) factor(x))) |>
  ...
  else if (stat %in% c("bin2d", "bin_2d", "binhex", "density2d", "density_2d", "density2d_filled", "density_2d_filled")) {
    if (!x_null & !y_null) {
      plot <- data %>%
        ggplot2::ggplot(mapping = ggplot2::aes(
          x = !!x,
