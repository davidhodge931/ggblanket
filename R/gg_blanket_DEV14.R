...
#'
#'   # print(col_map_or_set)
#'   # print(colour_map_or_set)
#'   # print(fill_map_or_set)
#'   # return()
#'
...
#'           colour_palette_discrete <- vapply(col_palette_discrete, border_colour, character(1))
#'         } else {
...
#'           # For vector palettes, apply border_colour to each color
#'           colour_palette_continuous <- vapply(col_palette_continuous, border_colour, character(1))
#'         } else {
...
#'           # For vector palettes, apply border_colour to each color
#'           colour_palette_ordinal <- vapply(col_palette_ordinal, border_colour, character(1))
#'         } else {
...
#'           fill_palette_discrete <- vapply(col_palette_discrete, border_fill, character(1))
#'         } else {
...
#'         } else if (is.character(col_palette_continuous)) {
#'           # For vector palettes, apply border_fill to each color
#'           fill_palette_continuous <- vapply(col_palette_continuous, border_fill, character(1))
#'         } else {
...
#'         } else if (is.character(col_palette_ordinal)) {
#'           # For vector palettes, apply border_fill to each color
#'           fill_palette_ordinal <- vapply(col_palette_ordinal, border_fill, character(1))
#'         } else {
...
#'       }
#'     } else if (col_map_or_set$is_aesthetic && !is.null(colour) && !colour_map_or_set$is_aesthetic) {
#'       # col is mapped, but colour is explicitly set to a fixed value - DO NOT OVERRIDE
#'       # This case is already handled above, but making it explicit
#'     } else {
...
#'       }
#'     } else if (col_map_or_set$is_aesthetic && !is.null(fill) && !fill_map_or_set$is_aesthetic) {
#'       # col is mapped, but fill is explicitly set to a fixed value - DO NOT OVERRIDE
#'       # This case is already handled above, but making it explicit
#'     } else {
...
#'   # UNLESS the user explicitly provided them as fixed values
#'   if (col_map_or_set$is_aesthetic) {
#'     # For colour: only remove if it wasn't explicitly set by the user
...
#'     }
#'     # For fill: only remove if it wasn't explicitly set by the user
...
#'
#'   #something like this, but needs to take into account when colour/fill is set
#'   #to not make them NULL in this situation
#'   #
#'   #   if (col_map_or_set$is_aesthetic) {
#'   #     if (!colour_map_or_set$is_aesthetic) {
#'   #       #should be only if not set in the function (e.g. colour = "black")
#'   #       fixed_params$colour <- NULL
#'   #     }
#'   #     if (!fill_map_or_set$is_aesthetic) {
#'   #       #should be only if not set in the function (e.g. fill = "grey")
#'   #       fixed_params$fill <- NULL
#'   #     }
#'   #   }
#'
