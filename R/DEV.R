    ...
    if (y_forcat) {
      ...
      if (rlang::is_null(y_labels)) {
        if (y_numeric | y_null) {
          ...
          # else {
          #   if (!facet_null & !facet2_null) y_labels <- \(x) str_keep_seq(scales::number(x, big.mark = ","))
          #   else if (facet_layout == "wrap") {
          #     if (facet_null & facet2_n >= 4) y_labels <- \(x) str_keep_seq(scales::number(x, big.mark = ","))
          #     else if (facet2_null & facet_n >= 4) y_labels <- \(x) str_keep_seq(scales::number(x, big.mark = ","))
          #     else y_labels <- \(x) scales::number(x, big.mark = ",")
          #   }
          #   else if (facet_layout == "grid") {
          #     if (facet2_null) y_labels <- \(x) scales::number(x, big.mark = ",")
          #     else if (facet_null) y_labels <- \(x) str_keep_seq(scales::number(x, big.mark = ","))
          #     else y_labels <- \(x) str_keep_seq(scales::number(x, big.mark = ","))
          #   }
          # }
        }
