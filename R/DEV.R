...
gg_blanket <- function(
  ...
  if (!stringr::str_detect(stat_name, "sf")) {
    ...
    if (x_numeric) {
        plot <- plot +
          ggplot2::scale_x_continuous(limits = x_limits1, oob = x_oob)
    }
    else if (x_date) {
        plot <- plot +
          ggplot2::scale_x_date(limits = x_limits1, oob = x_oob)
    }
    else if (x_posixct) {
        plot <- plot +
          ggplot2::scale_x_datetime(limits = x_limits1, oob = x_oob)
    }
    else if (x_hms) {
        plot <- plot +
          ggplot2::scale_x_continuous(limits = x_limits1, oob = x_oob)
      ...
      if (!rlang::is_null(x_limits1)) {
        plot <- plot +
          ggplot2::scale_x_discrete(limits = x_limits1, drop = x_drop)
      }
      else {
        plot <- plot +
    ...
    if (y_numeric) {
        plot <- plot +
          ggplot2::scale_y_continuous(limits = y_limits1, oob = y_oob)
    }
    else if (y_date) {
        plot <- plot +
          ggplot2::scale_y_date(limits = y_limits1, oob = y_oob)
    }
    else if (y_posixct) {
        plot <- plot +
          ggplot2::scale_y_datetime(limits = y_limits1, oob = y_oob)
    }
    else if (y_hms) {
        plot <- plot +
          ggplot2::scale_y_continuous(limits = y_limits1, oob = y_oob)
        ...
        plot <- plot +
          ggplot2::scale_y_discrete(limits = y_limits1, drop = y_drop)
    }
  }
    ...
    #get x_limits, x_expand and x_breaks for complex situation
      ...
      #get a vector to use
        ...
        if (ncol(x_vctr_temp) != 0) {
          # if (stringr::str_detect(stat_name, "bin")) {
          #   x_vctr <- x_vctr_temp %>%
          #     dplyr::select(tidyselect::matches(stringr::regex("^x$"))) %>%
          #     tidyr::pivot_longer(cols = tidyselect::everything()) %>%
          #     mutate(across(value, \(x) x * -1)) |>
      ...
      #get x_expand - or stio if it won't work
  ...
  if (stringr::str_detect(stat_name, "sf")) {
    ...
    #get y_limits, y_expand and y_breaks for complex situation
    else {
      ...
      #get y_expand - or stio if it won't work
