...
#'     col_scale_class, border, theme_palettes,
...
#'       if (border && !is.null(getOption("ggblanket.colour_palette_d_border"))) {
...
#'       if (border && !is.null(getOption("ggblanket.fill_palette_d_border"))) {
...
#'     # Continuous palettes
#'     if (is.null(colour_palette)) {
#'       if (border && !is.null(getOption("ggblanket.colour_palette_c_border"))) {
...
#'
#'     if (is.null(fill_palette)) {
#'       if (border && !is.null(getOption("ggblanket.fill_palette_c_border"))) {
...
#'           if (is.character(eval_value) && length(eval_value) == 1 && !inherits(eval_value, "AsIs")) {
...
#'               length(eval_value) == 1 && !inherits(eval_value, "AsIs")) {
#'             return(list(is_aesthetic = FALSE, value = eval_value))
#'           } else if (length(eval_value) == 1 && is.na(eval_value)) {
#'             return(list(is_aesthetic = FALSE, value = eval_value))
#'           }
...
#'   }
#'
#'   # Add guides based on what's actually mapped
#'   # Check if colour and fill are mapped to the same variable
#'   colour_label <- plot_build$plot$labels$colour
#'   fill_label <- plot_build$plot$labels$fill
#'
#'   if (!is.null(colour_label) && !is.null(fill_label) &&
#'       identical(colour_label, fill_label)) {
#'     # Same variable mapped to both - typically show both for discrete
#'     plot <- plot +
#'       ggplot2::guides(
#'         colour = ggplot2::guide_legend(
...
#' }
#'
#' # 1. Single correct version of check_same_colour_fill_mapping
...
#' }
#'
#' # 2. Correct ordinal scale function with plot_build parameter
#' #' Add ordinal color scale
#' #' @noRd
#' add_col_scale_ordinal <- function(
#'     plot, aes_list, data, plot_data,
...
#'
#'   # Add guides based on what's actually mapped
#'   # Check if colour and fill map to the same variable using the plot object
#'   same_mapping <- check_same_colour_fill_mapping(plot)
#'
#'   if (same_mapping) {
#'     # Same variable mapped to both - show both guides for ordinal
...
#' }
#'
#' # 3. Updated add_col_scale_continuous with consistent use of check_same_colour_fill_mapping
#' #' Add continuous color scale
#' #' @noRd
#' add_col_scale_continuous <- function(
#'     plot, colour_palette, fill_palette,
...
#'
#'     # Check if colour and fill map to the same variable
#'     # Use the plot object to check the actual final mapping
#'     same_mapping <- check_same_colour_fill_mapping(plot)
#'
#'     if (same_mapping) {
#'       # Same variable mapped to both - hide one guide
...
#'     } else {
#'       # Different variables or only one is mapped - show appropriate guides
#'       # Check what's actually mapped in the plot
#'       plot_mapping <- plot$mapping
#'       has_colour_mapping <- !is.null(plot_mapping$colour)
#'       has_fill_mapping <- !is.null(plot_mapping$fill)
#'
...
#'
#'     # Check if colour and fill map to the same variable
#'     # Use the same consistent approach as gradient
#'     same_mapping <- check_same_colour_fill_mapping(plot)
#'
#'     if (same_mapping) {
#'       # Same variable mapped to both - hide one guide
...
#' }
#'
#' # Helper function to check if colour and fill map to the same variable
#' #' Check if two aesthetic mappings are identical
#' #' @noRd
#' are_aesthetics_identical <- function(aes1, aes2, mapping = NULL) {
#'   # First check direct aesthetic mappings
#'   if (!rlang::quo_is_null(aes1) && !rlang::quo_is_null(aes2)) {
#'     expr1 <- rlang::quo_text(aes1)
#'     expr2 <- rlang::quo_text(aes2)
#'     if (identical(expr1, expr2)) {
#'       return(TRUE)
#'     }
#'   }
#'
#'   # If mapping is provided, check if both colour and fill are in mapping
#'   # and if they're the same
#'   if (!is.null(mapping)) {
#'     if ("colour" %in% names(mapping) && "fill" %in% names(mapping)) {
#'       # Convert mapping aesthetics to text for comparison
#'       colour_expr <- rlang::quo_text(mapping$colour)
#'       fill_expr <- rlang::quo_text(mapping$fill)
#'       return(identical(colour_expr, fill_expr))
#'     }
#'   }
#'
...
#' }
#'
#' # Add this helper function to check if an aesthetic is in mapping
#' #' Check if an aesthetic is mapped in the mapping argument
#' #' @noRd
