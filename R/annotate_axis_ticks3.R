...
#' #' @param position The position of the axis. One of "bottom", "top", "left" or "right". Auto-detected if NULL.
...
#'   }
#'
#'   # Determine breaks and position
#'   if (!rlang::is_null(x_breaks)) {
#'     breaks <- x_breaks
#'     is_x_axis <- TRUE
#'     if (rlang::is_null(position)) position <- "bottom"
#'   } else if (!rlang::is_null(y_breaks)) {
#'     breaks <- y_breaks
#'     is_x_axis <- FALSE
#'     if (rlang::is_null(position)) position <- "left"
#'   } else {
#'     return(list())
#'   }
#'
...
#'     }
#'     stamp <- c(stamp, list(theme_element))
