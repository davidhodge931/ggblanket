...
#' multiply and screen modes: multiplies dark colors and screens light colors,
...
.apply_blend_overlay <- function(col1, col2) {
  ...
  for (i in 1:3) {
    ...
    # This is based on the destination color (c2) determining the blend mode

    # Calculate overlay blend for each pixel
    overlay_blend <- ifelse(
      c2 <= 0.5,
      2 * c1 * c2,  # Multiply for dark colors
      1 - 2 * (1 - c1) * (1 - c2)  # Screen for light colors
    )
...
#' color rather than the destination, creating a more intense contrast effect.
#'
#' @param ... Either one or two colour arguments:
#'   - If one argument: the colour is blended with itself
#'   - If two arguments: the first is blended with the second
...
.apply_blend_hard_light <- function(col1, col2) {
  ...
  for (i in 1:3) {
    ...
    # This is based on the source color (c1) determining the blend mode

    # Calculate hard light blend for each pixel
    hard_light_blend <- ifelse(
      c1 <= 0.5,
      2 * c1 * c2,  # Multiply for dark source colors
      1 - 2 * (1 - c1) * (1 - c2)  # Screen for light source colors
    )
...
#' Color dodge blend colours with proper alpha compositing
#'
#' @description
#' Blends colours using color dodge mode with proper alpha handling. Color dodge
#' brightens the destination color to reflect the source color, creating
...
#' Internal color dodge blend implementation
#'
#' @param col1 Character vector of colours
#' @param col2 Character vector of colours
#' @return Character vector of color-dodge-blended colours
#' @noRd
.apply_blend_colour_dodge <- function(col1, col2) {
  ...
  for (i in 1:3) {
    ...
    # Apply color dodge blend formula
    # f(c1, c2) = c2 / (1 - c1)
    # Clamp to 1 when c1 approaches 1

    color_dodge_blend <- ifelse(
      c1 >= 1,
    ...
    result[idx_valid] <- (
      c1[idx_valid] * alpha1[idx_valid] * (1 - alpha2[idx_valid]) +  # Source only
        c2[idx_valid] * alpha2[idx_valid] * (1 - alpha1[idx_valid]) +  # Dest only
        color_dodge_blend[idx_valid] * alpha1[idx_valid] * alpha2[idx_valid]  # Color dodge blend
    ) / alpha_result[idx_valid]
...
#' Color burn blend colours with proper alpha compositing
#'
#' @description
#' Blends colours using color burn mode with proper alpha handling. Color burn
#' darkens the destination color to reflect the source color, creating
...
#' Internal color burn blend implementation
#'
#' @param col1 Character vector of colours
#' @param col2 Character vector of colours
#' @return Character vector of color-burn-blended colours
#' @noRd
.apply_blend_colour_burn <- function(col1, col2) {
  ...
  for (i in 1:3) {
    ...
    # Apply color burn blend formula
    # f(c1, c2) = 1 - (1 - c2) / c1
    # Return 0 when c1 approaches 0

    color_burn_blend <- ifelse(
      c1 <= 0,
    ...
    result[idx_valid] <- (
      c1[idx_valid] * alpha1[idx_valid] * (1 - alpha2[idx_valid]) +  # Source only
        c2[idx_valid] * alpha2[idx_valid] * (1 - alpha1[idx_valid]) +  # Dest only
        color_burn_blend[idx_valid] * alpha1[idx_valid] * alpha2[idx_valid]  # Color burn blend
    ) / alpha_result[idx_valid]
