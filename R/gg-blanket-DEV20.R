gg_blanket <- function(data,
  ...
  # Validate ggplot argument
  is_ggplot2 <- identical(ggplot, ggplot2::ggplot)
  is_ggraph <- identical(ggplot, ggraph::ggraph)

  if (!is_ggplot2 && !is_ggraph) {
    rlang::abort(message = "`ggplot` must be either `ggplot2::ggplot` or `ggraph::ggraph`.")
  }
  ...
  ### aesthetics
  if (identical(ggplot, ggplot2::ggplot)) {
      ...
      if (colour_type == "continuous") {
        plot <- plot +
          ggplot2::continuous_scale(
            ...
            breaks = colour_breaks %||% fill_breaks,
            guide = colour_guide %||% fill_guide %||% ggplot2::guide_legend(),
            labels = colour_labels %||% fill_labels %||% get_labels(coord_type, colour_subtype, "colour"),
            limits = colour_limits %||% fill_limits,
            name = colour_name %||% fill_name,
