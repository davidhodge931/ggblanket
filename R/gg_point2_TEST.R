    ...
    if (is.character(rlang::eval_tidy(x, data)) | is.factor(rlang::eval_tidy(x, data))) {
      ...
      if (facet_scales %in% c("fixed", "free_y")) {
        ...
        if (rlang::is_null(x_limits) & rlang::is_null(x_breaks)) {
          ...
          else if (x_numeric) x_breaks <- scales::breaks_extended(n = x_breaks_n)(x_range)
          else if (x_date) x_breaks <- scales::breaks_pretty(n = x_breaks_n)(x_range)

          if (x_trans %in% c("log10", "log2", "log", "sqrt")) x_limits <- x_range
          ...
          else if (x_numeric) x_breaks <- scales::breaks_extended(n = x_breaks_n)(x_limits)
          else if (x_date) x_breaks <- scales::breaks_pretty(n = x_breaks_n)(x_limits)
  ...
  if (!rlang::quo_is_null(y) & rlang::quo_is_null(x)) {
    if (is.character(rlang::eval_tidy(y, data)) | is.factor(rlang::eval_tidy(y, data))) {
      ...
      if (facet_scales %in% c("fixed", "free_x")) {
        ...
        if (rlang::is_null(y_limits) & rlang::is_null(y_breaks)) {
          ...
          else if (y_numeric) y_breaks <- scales::breaks_extended(n = y_breaks_n)(y_range)
          else if (y_date) y_breaks <- scales::breaks_pretty(n = y_breaks_n)(y_range)

          if (y_trans %in% c("log10", "log2", "log", "sqrt")) y_limits <- y_range
          ...
          else if (y_numeric) y_breaks <- scales::breaks_extended(n = y_breaks_n)(y_limits)
          else if (y_date) y_breaks <- scales::breaks_pretty(n = y_breaks_n)(y_limits)
