...
set_blanket <- function(
  ...
  geom_theme <- rlang::set_names(
    ...
    if (rlang::is_null(fill_palette)) {
      ...
      fill_palette_c <- (scales::pal_viridis(option = "mako", begin = 0.1, end = 0.9))
    ...
    # KEY FIX: When palette is a function, create a wrapper that applies border_pop
    # to the OUTPUT of the palette function, not to the function itself
    if (is.function(fill_palette_d)) {
      colour_palette_d <- function(n) {
        colors <- fill_palette_d(n)
        border_pop(colors)
      }
    } else {
    ...
    if (is.function(fill_palette_c)) {
      colour_palette_c <- function(x) {
        colors <- fill_palette_c(x)
        border_pop(colors)
      }
    } else {
