#' Qq ggplot
#'
#' @description Create a qq ggplot with a wrapper around [ggplot()][ggplot2::ggplot()] + [layer()][ggplot2::layer()] with [geom_qq_line()][ggplot2::geom_qq_line()] defaults for the geom, stat and position.
#'
...
gg_qq_line <- function(
  ...
  gg_blanket(
    ...
    z = {{ z }},
    col = {{ col }},
    colour = {{ colour }}, fill = {{ fill }}, shape = {{ shape }}, linetype = {{ linetype }}, facet = {{ facet }},
    facet2 = {{ facet2 }},
    group = {{ group }},
    subgroup = {{ subgroup }},
    label = {{ label }},
