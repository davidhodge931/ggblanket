...
#'   fixed_params <- list()
#'
#'   # # Define geoms that compute their own colour/fill aesthetics
#'   # stat_computed_geoms <- c("bin2d", "hex", "contour_filled", "density2d_filled")
#'   #
#'   # # Handle colour fixed value
#'   # # Set fixed colour if colour is not mapped as aesthetic
#'   # if (!colour_map_or_set$is_aesthetic) {
#'   #   # Skip setting fixed colour for stat-computed geoms unless explicitly provided
#'   #   if (!(geom %in% stat_computed_geoms && is.null(colour_map_or_set$value))) {
...
#'   #   # Skip setting fixed fill for stat-computed geoms unless explicitly provided
#'   #   if (!(geom %in% stat_computed_geoms && is.null(fill_map_or_set$value))) {
...
#'   # Set fixed colour if colour is not mapped as aesthetic
#'   if (!colour_map_or_set$is_aesthetic) {
#'     # For stat-computed geoms, ONLY set if explicitly provided
#'     if (geom %in% stat_computed_geoms) {
#'       if (!is.null(colour_map_or_set$value)) {
#'         fixed_params$colour <- colour_map_or_set$value
#'       }
#'       # Otherwise don't set colour at all - let the stat compute it
#'     } else {
#'       # Normal geoms - apply full logic
#'       if (!is.null(colour_map_or_set$value)) {
#'         # Explicit colour value provided - use as is (no border transformation)
#'         fixed_params$colour <- colour_map_or_set$value
#'       } else if (!col_map_or_set$is_aesthetic && !is.null(col_map_or_set$value)) {
...
#'         fixed_params$fill <- fill_map_or_set$value
#'       }
#'       # Otherwise don't set fill at all - let the stat compute it
#'     } else {
#'       # Normal geoms - apply full logic
#'       if (!is.null(fill_map_or_set$value)) {
#'         # Explicit fill value provided - use as is (no border transformation)
