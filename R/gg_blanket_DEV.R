...
#'   }
#'
#'   # Step 1: Detect aesthetic vs fixed for col, colour, fill, shape, linetype
#'   col_detection <- detect_aesthetic_or_value(rlang::enquo(col), "col")
#'   colour_detection <- detect_aesthetic_or_value(rlang::enquo(colour), "colour")
#'   fill_detection <- detect_aesthetic_or_value(rlang::enquo(fill), "fill")
#'   shape_detection <- detect_aesthetic_or_value(rlang::enquo(shape), "shape")
#'   linetype_detection <- detect_aesthetic_or_value(rlang::enquo(linetype), "linetype")
#'
...
#'   )
#'
#'   # Keep track of original col aesthetic for scale determination
#'   aes_list$col <- if (col_detection$is_aesthetic) col_detection$value else rlang::quo(NULL)
#'
#'   # Determine colour aesthetic
#'   if (colour_detection$is_aesthetic && !is.null(colour_detection$value)) {
#'     aes_list$colour <- colour_detection$value
#'   } else if (col_detection$is_aesthetic && !is.null(col_detection$value)) {
#'     aes_list$colour <- col_detection$value
#'   } else {
...
#'     aes_list$fill <- fill_detection$value
#'   } else if (col_detection$is_aesthetic && !is.null(col_detection$value)) {
#'     aes_list$fill <- col_detection$value
#'   } else {
#'     aes_list$fill <- rlang::quo(NULL)
#'   }
#'
#'   # Shape and linetype aesthetics
#'   aes_list$shape <- if (shape_detection$is_aesthetic) shape_detection$value else rlang::quo(NULL)
#'   aes_list$linetype <- if (linetype_detection$is_aesthetic) linetype_detection$value else rlang::quo(NULL)
#'
...
#'
#'   # position <- extract_ggproto_name(position, "Position")
#'
