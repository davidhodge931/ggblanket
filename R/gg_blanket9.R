...
gg_blanket <- function(
  ...
  if (shape_mapped && !is.null(shape_palette)) {
    ...
    shape_equals_y <- FALSE
    if (!rlang::quo_is_null(aes_list$y) && !rlang::quo_is_null(aes_list$shape)) {
      shape_equals_col <- identical(
        rlang::eval_tidy(aes_list$shape, data),
    ...
    # Handle reversal for perspective = "y"
    if (perspective == "y") {
      if (!shape_equals_col && !shape_equals_y) {
        shape_values <- rev(shape_values)
        shape_legend_rev <- TRUE
      } else if (shape_equals_col && !shape_equals_y) {
        shape_legend_rev <- rev(col_legend_rev)
      } else if (shape_equals_col && shape_equals_y) {
  ...
  if (linetype_mapped && !is.null(linetype_palette)) {
    ...
    linetype_equals_y <- FALSE
    if (!rlang::quo_is_null(aes_list$y) && !rlang::quo_is_null(aes_list$linetype)) {
    ...
    # Handle reversal for perspective = "y"
    if (perspective == "y") {
      if (!linetype_equals_col && !linetype_equals_y) {
        linetype_values <- rev(linetype_values)
        linetype_legend_rev <- TRUE
      } else if (linetype_equals_col && !linetype_equals_y) {
        linetype_legend_rev <- rev(col_legend_rev)
      } else if (linetype_equals_col && linetype_equals_y) {
