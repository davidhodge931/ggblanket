...
gg_blanket <- function(
  ...
  is_fill_mapped <- "fill" %in%
    names(separated$mapped) ||
    "fill" %in% names(mapping)
  is_colour_mapped <- "colour" %in%
    names(separated$mapped) ||
    "colour" %in% names(mapping)
  is_linewidth_mapped <- "linewidth" %in%
    names(separated$mapped) ||
    "linewidth" %in% names(mapping)
  ...
  is_stroke_mapped <- "stroke" %in%
    names(separated$mapped) ||
    "stroke" %in% names(mapping)
  ...
  if (!is.null(computed_colour)) {
    all_params$colour <- computed_colour
  }
  if (!is.null(computed_linewidth)) {
    all_params$linewidth <- computed_linewidth
  }
  if (!is.null(computed_stroke)) {
      ...
      if (fill_type == "continuous") {
        plot <- plot +
          ggplot2::continuous_scale(
            ...
            labels = fill_labels %||%
              get_labels(coord_type, fill_subtype, "fill"),
            limits = fill_limits,
            name = fill_name,
            oob = fill_oob,
  ...
  if (!is.null(colour_type)) {
    if (colour_type == "discrete") {
      ...
      if (is_named_palette(colour_palette)) {
        colour_values <- if (
          is.null(names(colour_palette)) && is_named_palette(fill_palette)
        ) {
  ...
  if (x_type == "discrete" & y_type == "discrete") {
    discrete <- "both"
