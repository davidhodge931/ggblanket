...
#' #'       plot, palettes, border, col_breaks, col_breaks_n,
...
#' #' @param shape,linetype,linewidth,size,alpha Can be either aesthetic or fixed value
...
#' #'   }
#' #'
#' #'   # Add colour if provided (it already includes col inheritance from aes_list)
#' #'   if (!rlang::quo_is_null(aes_list$colour)) {
#' #'     base_aes <- utils::modifyList(
#' #'       base_aes,
#' #'       ggplot2::aes(colour = !!aes_list$colour)
...
#' #'   }
#' #'
#' #'   # Add fill if provided (it already includes col inheritance from aes_list)
#' #'   if (!rlang::quo_is_null(aes_list$fill)) {
#' #'     base_aes <- utils::modifyList(
#' #'       base_aes,
#' #'       ggplot2::aes(fill = !!aes_list$fill)
...
#' #'   }
#' #'
#' #'   # Note: We don't need to handle col separately here because
#' #'   # it's already been inherited by colour and fill in aes_list
#' #'
#' #'   # Merge with additional mapping
#' #'   final_aes <- if (!is.null(mapping)) {
#' #'     utils::modifyList(base_aes, mapping)
