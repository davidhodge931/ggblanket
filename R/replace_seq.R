...
#'
#' @description Replace elements in a vector sequence.
#'
#' @param x A vector.
...
#' @param value The value to replace non-kept elements with. Defaults to `""`.
#'
#' @return A vector.
#' @export
#'
...
replace_seq <- function(x,
  ...
  if (is.numeric(x)) {
    replace(scales::comma(x, ...),
            ...
            %% keep_nth != (offset + 1), value)
  }
  else if (lubridate::is.Date(x) | lubridate::is.POSIXct(x) | hms::is_hms(x)) {
    replace(as.character(x), seq_along(as.character(x)) %% keep_nth != (offset + 1), value)
  }
  else {
    replace(x, seq_along(x) %% keep_nth != (offset + 1), value)
  }
}
