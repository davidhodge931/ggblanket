...
There are no `*_limits` arguments in ggblanket. Instead, users should use a combination of filtering the data, and adding `*_expand_limits` and ``coord_cartesian(xlim = ..., ylim = ...)` arguments etc. In some situations, the `*_breaks` will need to be updated.

```{r}
#where the stat is "identity"...
penguins |>
...
```{r}
#where the stat is _not_ "identity", zoom out as follows:
penguins |>
  gg_smooth(
    x =  body_mass_g, 
    y = bill_depth_mm,
...
```{r}
#where the stat is _not_ "identity", zoom in as follows: 
penguins |>
  gg_smooth(
    x =  body_mass_g, 
    y = bill_depth_mm,
    ...
    y_breaks = scales::breaks_width(0.05), #recreate breaks on symmetric axis
  ) 
```
