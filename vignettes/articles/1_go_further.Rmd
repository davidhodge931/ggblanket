...
This article will demonstrate a random assortment of content, including some of which is morre advanced.  
...
7. Correct the default `*_orientation`
...
12. Use opacity with to emphasise/demphasise. 
13. Understand `oob` & `clip`
...
Where `*_orientation = TRUE`, it will make relevant linear continuous scales ‘symmetric’ where possible with (a) `*_limits` the range of `*_breaks` and (b) `*_expand = c(0, 0)`. 

But you can avoid this by adding a `*_limits = c(NA, NA)` argument (or `*_limits = c(0, NA)` for bars).
...
### 12. Use opacity to emphasise/demphasise 
