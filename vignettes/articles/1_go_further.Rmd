...
7. Transform scales to `"log"` etc
8. Change the `*_position` of positional axes 
9. Add contrasting dark or light text on polygons
10. Use opacity with to emphasise/demphasise. 
11. Centre a diverging colour scale
12. Use `replace_seq` to keep every nth label
13. Understand `oob` & `clip`
14. Use delayed evaluation
15. Use character inputs of variables in shiny apps
16. Make custom `gg_*` wrapper functions
...
You may wish to make your own wrapper functions if you are continually adding the same defaults to your `gg_*` functions. 

This essentially allows you to choose your own defaults in the functions.

Use the `...` argument to access all other arguments in the `gg_*` function.

An alternative method to `enquo` and `!!` is to use `{{ }}`.
...
gg_point_custom <- function(data, 
                            ...
                            shape = 17,
                            size = 3,
                            titles_to_case = snakecase::to_title_case,
                            ...) {
  ...
  if (rlang::is_null(col_pal)) {
    if (rlang::quo_is_null(col)) col_pal <- plum
    else col_pal <- RColorBrewer::brewer.pal(n = 8, "Dark2")
  }
