...
1. Use `shape`, `linetype`, `linewidth` or `size` aesthetics
...
Most functions have the commonly needed aesthetics directly available as an argument. For those that are _not_ available, you can either use `mapping` argument. The `mapping` argument provides access to the `size`, `shape`, `linetype` and `linewidth` aesthetics within the `gg_*` function. Note you may have to change the title by adding a `ggplot2::labs` layer, or change the legend values by adding a  `ggplot2::scale_*_manual` layer etc.
