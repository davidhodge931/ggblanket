...
10. Use `replace_seq` to keep every nth label
11. Understand `oob` & `clip`
12. Use delayed evaluation
13. Work with `coord_radial()`
...
p4 <- penguins |> 
  ...
  gg_density(
    ...
    x_breaks = scales::breaks_pretty(n = 3),
    y_labels = replace_seq,
    col_palette = c(orange, scales::alpha("#78909C", 0.25)),
...
ggblanket modes are not really built with a `coord_radial` cartesian system in mind. However, with some tweaking to the `axis.text.x` and `axis.title.y`, you can get something that looks okay.
...
penguins |> 
  ...
  theme(axis.text.x = element_text(margin = margin(rep(base_size / 3, 4)))) +
  theme(axis.title.y = element_text(hjust = 0.885))
```
