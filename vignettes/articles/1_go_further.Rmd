...
3. Change the `stat`
4. Fix either the colour outline or interior fill to a constant colour
5. Use non-supported aesthetics
6. Ability to set the mode globally
...
You can set the mode globally using `mode_set` (e.g. `mode_set(dark_mode_rt())`). This will result in plots where the `mode` argument is `NULL` using the set mode. You can unset it using `mode_set(grey_mode_rt())`.

You can also set a `theme` globally that does not have side-effects within the `gg_*` function using `ggplot2::theme_set()`. This will in effect result in the set theme `+`-ed on as a layer to every plot. You can unset this using `ggplot2::theme_set(theme_grey())`. Note if you want to set the theme to actually apply `ggplot2::theme_grey()`, then you need to modify the base_size slightly (e.g `ggplot2::theme_set(theme_grey(11.01))`). 
