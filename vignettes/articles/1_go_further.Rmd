...
8. Use `replace_seq` to keep every nth label 
9. Use the `flipped` argument
10. Use delayed evaluation
11. Understand `oob` & `clip`
12. Centre a diverging colour scale 
13. Transform scales
...
Sometimes the plot might guess the `flipped`-ness of the plot incorrectly - which affects the axis-line/ticks and gridlines removal. In these situations, you can specify whether `flipped = TRUE` or `flipped = FALSE`.

If it's still not right in terms of axis-lines and gridlines, you can `+ *_mode_*` on, and then remove any unwanted gridlines and axis-line/ticks.
...
Transform objects (e.g. `scales::transform_log()` or character strings of these can be used to transform scales - inncluding combining these.
