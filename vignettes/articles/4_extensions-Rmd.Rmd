...
penguins2 |> 
  gg_blank(x = species, 
           y = body_mass_g) + 
  ggbeeswarm::geom_quasirandom(colour = mako(9)[5])
```
...
penguins2 |>
  gg_blank(x = flipper_length_mm,
           y = body_mass_g,
           facet = species) +
  ggdensity::geom_hdr(colour = NA, fill = mako(9)[5]) +
  labs(alpha = "Probs")
```
...
d |>
  ...
  ggdist::stat_slabinterval(
    aes(dist = dist),
    colour = mako(9)[5],
    fill = scales::alpha(mako(9)[5], 0.5))
```
...
p1 <- pressure |>
  ...
  geom_textline(
    ...
    size = 3.53, vjust = -0.5, hjust = 0.9, color = mako(9)[5], linecolor = mako(9)[5])
