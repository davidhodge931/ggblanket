...
penguins2 |> 
  gg_blank(x = species, 
           y = body_mass_g) + 
  ggbeeswarm::geom_quasirandom(colour = viridis::mako(9)[5])
```
...
data.frame(x = rnorm(1000),
  ...
  gg_blanket(
    ...
    x = x, 
    y = y, 
    #this method of removing colour is better for the lenend
    col_pal = scales::alpha(viridis::mako(9)[5], 0),   
    title = "ggdensity::geom_hdr"
  ) 
```
...
penguins2 |> 
  ...
  geom_point(aes(x = flipper_length_mm, y = body_mass_g, colour = species), 
             data = penguins2 |> filter(species != "Gentoo")) +
  scale_color_manual(values = pal_guardian[c(3,2)]) +
  labs(colour = "Species")
```
...
mtcars |>
  ...
  gg_point(x = wt, y = mpg, label = car) +
  ggrepel::geom_text_repel(colour = "#2B6999", size = 3.53) 
```
...
p1 <- pressure |>
  mutate(label = "Mercury vapor pressure increases with temperature") |>
  gg_blanket(
    GeomTextpath,
    x = temperature,
    y = pressure,
...
p <- diamonds |>
  ...
  ggiraph::geom_bar_interactive(
    aes(tooltip = after_stat(count), 
        data_id = color), 
    width = 0.75, 
    col = viridis::mako(9)[5], 
    fill = viridis::mako(9)[5], 
    alpha = 0.9)
...
Using `gg_blank` with the `text` aesthetic provides most flexibility over  tooltips. Alternatively, you can use any `gg_*` function with  `plotly::ggplotly(p, tooltip = c("x", "y", "fill"))` or `plotly::ggplotly(p)`. 
