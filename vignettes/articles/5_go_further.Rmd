...
An axis line can be easily replaced by a new annotated axis line, which can sometimes be useful. 
...
penguins |>
  ...
  gg_point(
    ...
    y = body_mass_g,
    col = sex,
    annotate = annotate_axis_line(position = "bottom", theme = "transparent"),
  ) 
```
...
palmerpenguins::penguins |> 
  ...
  geom_jitter(shape = 19, colour = col_multiply(jumble[9])) 
```
...
d |>
  gg_col(
    ...
    xmax = upper,
    col = sex,
    x_limits_include = max(d$upper, na.rm = TRUE), 
    width = 0.75,
    position = position_dodge(),
  ) +
  geom_errorbar(
