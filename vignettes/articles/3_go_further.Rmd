...
* Correct the default perspective
...
The `gg_*` function guesses the `perspective` of the plot to determine how to make continuous axes and what side-effects to have on the provided theme. If it guesses incorrectly, use either the `perspective` argument.
...
mpg  |>  
  gg_point(
    ...
    colour_palette = c(navy, teal, "white", orange, red),
    theme = theme_darker(),
  )
```
