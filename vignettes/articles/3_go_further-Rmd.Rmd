...
Text labels can be coloured based on the lightness/darkness of underlying polygons using a method developed by @teunbrand [here](https://github.com/teunbrand/ggplot_tricks). It requires the  `show.legend = FALSE` argument in `geom_text` to work.   
...
d <- cor(mtcars) |>
  ...
  tibble::tibble() |> 
  tidyr::pivot_longer(mpg:carb, names_to = "colname") |>
  mutate(across(c(rowname, colname), str_to_sentence)) 
