...
So with `gg_blank`, you may need to include the ggplot2 `stat` and `position` for plots that require different x and y scales to those created by a stat and position of `identity`. The `stat` must be a character, as stat functions are not supported. The `position` can be character or function.
...
penguins |>
  gg_blank(
    x = body_mass_g, 
    stat = "bin") +
  geom_histogram(colour = pal_blue, fill = pal_blue, alpha = 0.9)
```

### 2. Modifying a part of the colouring 

Where a `gg_*` function contains a geom that can colour both outlines and fill, users can turn off a part of this using `alpha = 0` or `pal = scales::alpha("red", 0)`. 
...
If a `col` variable is specified, then you can _also_ use `colour = "black"` or `fill = "lightgrey"` to fix a part of the colouring to a single fixed colour (or `colour = NA` or `fill = NA`). If you need to do this where a col variable is not specified, you can use `gg_bblank`.  
...
p1 <- iris |>
  gg_blank(
    x = Sepal.Length,
    stat = "density") +
  geom_density(colour = "black", fill = pal_blue, alpha = 0.5)

p2 <- iris |>
  gg_blank(
    x = Sepal.Length,
    stat = "density") +
  geom_density(colour = pal_blue, fill = "lightgrey", alpha = 0.5)
...
p1 <- penguins |>
  gg_blank(
    x = bill_length_mm,
    y = body_mass_g) +  
  geom_point(aes(alpha = bill_depth_mm), colour = "#2B6999") +
...
A method described by @teunbrand [here](https://github.com/teunbrand/ggplot_tricks) can be used to add text to polygons that is either black or white dependent on the colour of the polygon. 

```{r}
#choose any dark and light colours that go with your theme
contrast <- function(colour) {
  out   <- rep("#121B24", length(colour)) #use "#1f2f3e" for dark_mode
  light <- farver::get_channel(colour, "l", space = "hcl")
  out[light < 50] <- "#fcfdfe" #use "#bbccdd" for dark_mode 
  out
}
...
penguins |>
  ...
  gg_col(
    ...
    width = 0.75) +
  geom_text(
    aes(label = round(flipper_length_mm, 0),
      x = flipper_length_mm - (max(flipper_length_mm) * 0.05),!!!autocontrast),
    size = 3.66,
