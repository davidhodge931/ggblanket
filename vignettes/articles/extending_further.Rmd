...
8. Understand ggblanket uses scales::oob_keep by default
...
The `gg_*` function puts the aesthetics within the wrapped `ggplot` function. Therefore, the aesthetics will inherit to any subsequent geom's added by default. Geom's will plot in order. The `gg_*` function will plot the associated geom as the first layer, and then other geoms will be plotted on top of it. Note the `pal` also inherits to subsequent `geom_*` layers, but `alpha` does not. Use the `*_include` arguments as necessary to ensure the min and max of all layers are included in your numeric scales.
...
A key difference between ggplot2 and ggblanket alignment is the out-of-bound (oob) default used for continuous scales. ggblanket uses `scales::oob_keep`, whereas ggplot2 uses `scales::oob_censor`. Both use the default `coord_cartesian` function with the default `clip = "on"`, which clips anything that is out-of-bounds. Subsequently, plots where limits are set and the stat is _not_ identity may differ between ggblanket and ggplot2 - unless ggblanket defaults are changed to align with ggplot2 or vice versa (e.g. `*_oob = scales::oob_censor`). This ggblanket design decision was made to support the pretty defaults for symmetry that are discussed in the [Getting Started](https://davidhodge931.github.io/ggblanket/articles/ggblanket.html) article. 

```{r}
economics |> 
  ggplot() +
  geom_smooth(aes(x = date, y = unemploy)) 
```
...
economics |> 
  ggplot() +
  geom_smooth(aes(x = date, y = unemploy)) +
  scale_x_date(limits = c(lubridate::ymd("1985-01-01", "1995-01-01"))) 
```
...
economics |> 
  ...
  geom_smooth(aes(x = date, y = unemploy)) +
  scale_x_date(
    limits = c(lubridate::ymd("1985-01-01", "1995-01-01")),
    oob = scales::oob_keep) +
  geom_hline(yintercept = c(7000, 9000), linetype = 3)
```
