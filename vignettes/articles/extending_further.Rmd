...
7. Work with non-default stat's
...
The plot scales are constructed without knowledge of any layers subsequent to the `gg_*` function. The `gg_*` function builds the x and y scales based on the `data`, `stat`, `x`, and `y` in the `gg_*` function - and these can be adjusted using the `stat`, `*_include`, `*_limits` and `facet_scales` arguments.  
...
### 7. Work with non-default stats

The `stat` of the `gg_*` functions except `gg_blank` are locked to their   default in their equivalent `geom_*` function. Therefore if you want to use a different stat. Then you can use `gg_blank` with a character stat (e.g. "summary").  
...
It should be noted that the `gg_*` function builds the scale for the data that it thinks will be within the panel. Therefore in some situations, you may _need_ to add another ggplot scale to ensure the full height of the non-highlighted data is included. 
