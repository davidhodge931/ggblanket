...
p3 <- penguins |>
  gg_blank(
    ...
    col = species,
    col_legend_place = "r") + 
  geom_point(aes(alpha = flipper_length_mm)) +
  labs(alpha = "Flipper length mm") +
...
Where a `gg_*` function contains a geom that can colour both outlines and fill, users can turn off a part of this using the `alpha` and `pal` arguments. 

If it is desired to fix either the outline or fill to a single colour that is different to the `pal`, then users should instead use `gg_blank`.
...
The key to making this work is ensuring that `gg_blank` builds the distribution scale correctly. You can hack this by adding `*min` and `*max` aesthetics of a very low and very high quantile of the distribution.
...
data.frame(
  ...
  mutate(mindist = quantile(dist, 0.001)) |>
  mutate(maxdist = quantile(dist, 0.999)) |>
  gg_blank(
    y = name, 
    xmin = mindist,
    xmax = maxdist,
