...
The showtext and sysfonts packages support the use of different fonts from google. The `gg_theme_*` functions provide `text_family`, `title_family`, `subtitle_family` and `caption_family` arguments. Fonts can be identified on the Google fonts [website](https://fonts.google.com/).
...
penguins |>
  gg_point(
    ...
    theme = gg_theme_light(base_family = "grace",
                           base_size = 12))
...
penguins |>
  gg_blank(
    ...
    # theme = gg_theme_light(title_face = "plain")
    ) +
...
penguins |>
  ...
  theme(  
    ...
    strip.text.x = ggtext::element_markdown(margin = margin(b = 5)),
...
The ggeasy package provides a lot of support for easy modifying themes. 
...
p <- diamonds |>
  gg_bar(
    ...
    col = color,
    width = 0.75,
    theme = gg_theme_light(base_family = "helvetica")) 
...
p <- diamonds |>
  gg_blank(
    ...
    y_include = 0,
    stat = "count", 
    theme = gg_theme_light(base_family = "helvetica", base_size = 9)) +
