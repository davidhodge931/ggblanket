...
3. Modify a part of the colouring 
4. Use the mapping argument 
5. Change the default stat
...
The `mapping` argument provides additional flexibility to access the `linetype`, `shape` and `size` aesthetics. It also allows for positional `x` and `y` variables to be provided within a function, such as within `after_stat`. Note the `colour`, `fill` and `alpha` aesthetics cannot be added within the `mapping` argument. These plots could potentially be made using the `gg_blank` function and relevamt `geom_*` layer.
