...
5. Modifying a part of the colouring 
6. Use non-supported geoms
...
Where a `gg_*` function contains a geom that can colour both outlines and fill, users can turn off a part of this using the `alpha` and `pal` arguments as shown below. 
...
rbind(
  ...
  geom_point(size = 3, alpha = 0.5) * (ggblend::blend("lighten") + 
