...
6.  Prefixed arguments for titles, scales, guides, and faceting
7.  Unspecified x, y, and col titles converted to sentence case
...
Unspecified `x`, `y`, and `col` titles are converted to sentence case with snakecase::to_sentence. All titles can be manually changed using the `*_title` arguments. The default conversion is intended to make titles sometimes able to be left as is.
...
Where there are multiple geom layers in a desired plot, users should determine the `gg_*` function to use based on the following: 

1. Geoms will plot in order. The `gg_*` function will plot the associated geom as the first layer, and then other geoms will be plotted on top of it.

2. If some geom layers have a `col` aesthetic and some do not, then a `gg_*` function should be chosen that has a `col` argument in it. This will enable access to `col_*` arguments etc. It may be that `gg_blank` is required to make this work.
...
* `ggblanket::pal_discrete` for a discrete `col` variable with 5 or less levels (or unique values if a character). Testing suggests this is safe for colour blindness. 
