...
When `facet` is specified (and `facet2 = NULL`), the `facet_layout` will default to `"wrap"` (i.e. it will use `ggplot2::facet_wrap` under the hood).
...
When `facet2` is specified, the `facet_layout` will default to `"grid"` (i.e. it will use `ggplot2::facet_grid` under the hood).
