...
* scales are built within `gg_*()` and _without_ reference to later geom layers. Therefore, the `gg_*()` function should be relevant to a `geom_*()` that can _understand_ the aesthetics and other arguments needed to construct the scales correctly - and/or include appropriate `*_expand_limits` arguments. 

The `gg_blanket()` function with it's `"blank"` geom can be useful. 
