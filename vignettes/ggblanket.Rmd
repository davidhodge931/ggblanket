...
Use `mode_set(light_mode_rt())` or `theme_set(theme_grey())` to unset. Note you cannot actually set the theme globally to `theme_grey(11)` for this reason - instead use `theme_grey(11.01)`.
...
In general, users should use `ggplot2::labs` and `ggplot2::scale_*` layers to manipulate `size`, `shape`, `linetype` and `linewidth` aesthetics.
...
ggblanket requires unquoted variables only for `x`, `y`, `col`, `facet`, `facet2` and  `alpha`. You cannot wrap these in a function. Instead you may need to manipulate the data prior to plotting (e.g. using `forcats::fct_rev` and/or `forcats::fct_reorder`).
