...
In making a plot with multiple layers, users should consider:

* the order of `geom_*()` layers with the `gg_*()` appropriate to the bottom layer
* that scales are built within `gg_*()` and without reference to later layers. Subsequently, users must ensure that aesthetics are _understood_ by the `geom_*()` within the `gg_*()` function that cover the full range of the data. A `"blank"` geom with `gg_blanket()` can accommodate _all_ aesthetics, and such can be used to accomplish this. However, if this is used, users should also ensure that arguments such as `stat`, `position`, `facet`, `facet2`, and `*_expand_limits` etc are used as necessary.

For layers after the `gg_*()` function, users should also note that aesthetics directly added in the `gg_*` function are globally inherited. However, those added to the `mapping` argument are only available _locally_ to the layer associated with the `gg_*` function. 
