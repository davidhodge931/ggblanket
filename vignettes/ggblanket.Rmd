...
The scales are built within the `gg_*()` function _without_ knowledge of later geom layers. So, in some situations, users will need to take care to accommodate this within the `gg_*()` function. The `gg_*()` function builds scales with regard to the `stat`, `position`, and aesthetics (that the geom understands) etc.  
