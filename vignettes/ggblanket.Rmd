...
ggblanket provides two complete families of ggplot2 theme functions called `light_mode_*` (the default) and `dark_mode_*`. 

The suffix refers to the placement of the legend with `rt` referring to right top, `r` right centre, `b` being bottom and `t` being top.  
...
Note that `theme_set(theme_grey())` resets the set theme for ggplot2 code to theme_grey and for ggblanket `gg_*` functions to `light_mode_rt()`. If you want ggblanket `gg_*` functions to default to using `theme_grey()`, then you must modify the base_size slightly (e.g. `theme_set(theme_grey(11.01))`).
...
2. Add all of the aesthetics required into the `gg_*` function, including the `col` argument if this aesthetic is required in any of the later layers. If you do not want the first layer to have a colour or fill aesthetic, but you wish later layers to access this, use `gg_blank`.

3. Users need to ensure that the scales built by their `gg_*` function are appropriate for subsequent layers. Plot scales are built by the `gg_*` function based on the `data`, `x`, `y`, `*_limits`, `*_expand_limits`, `stat`, `position` and `coord` arguments in the `gg_*`  function. 
...
penguins2 |>
  ...
  mutate(lower = body_mass_g * 0.95) |> 
  mutate(upper = body_mass_g * 1.2) |> 
  gg_blank( #blank works better here, so that you can keep the col scale
    x = body_mass_g,
    y = species,
    col = species,
    xmin = lower, 
  ...
  geom_col(
    width = 0.75,
    colour = "#d3d3d3",#and turn it off here for this layer
    fill = "#d3d3d3",
    alpha = 0.9) +
