...
Users can make plots with multiple `ggplot2::geom_*` layers. The `gg_*()` geom layer will be the bottom geom layer of the plot, and each  subsequent `geom_*()` layer is placed on top. 
...
The scales are built within the `gg_*()` function _without_ knowledge of later geom layers. So, in some situations, users will need to take care to accommodate this within the `gg_*()` function. The `gg_*()` function will build scales only with regard to the `stat`, `position`, and aesthetics relevant to the geom (e.g. `geom_col()` does not understand the `xmax` aesthetic). The `*_expand_limits` 
...
(Note, any aesthetics added to the `mapping` argument are only available _locally_ to the geom layer associated with the `gg_*` function). 
