...
ggblanket keeps unused factor levels in the plot. If users wish to drop unused levels they should do by mutating the variable prior to plotting (i.e. `data |> mutate(var = forcats::fct_drop(var))`). Similarly, ggblanket requires unquoted variables for it's aesthetics. If you wish to reorder or reverse a categorical variable or drop NAs, you should do it likewise prior to plotting. 
...
ggblanket also aligns y character and factor y values from top to bottom, where ggplot2 does the opposite. This is with the intention that alphabetical order is maintained for characters (and factors with  default alphabetical levels). Where this is not what you want, reorder or reverse the variable prior to plotting. 
...
* `ggblanket::pal_discrete` for a discrete `col` variable with 5 or less levels (or unique values if not ordered). Testing suggests this is red/green colourblind-safe.
