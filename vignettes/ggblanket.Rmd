...
The `gg_*` function will guess the plot orientation based on whether the y axis is discrete (if not specified). 

Where `x_orientation = TRUE`, it will:

* remove any y axis line/ticks and the x gridlines from the `mode` theme. 
* make linear y continuous scales 'symmetric' where possible with (a) `y_limits` the range of `y_breaks` and (b) `y_expand = c(0, 0)`.

The vice versa will occur for when `y_orientation = TRUE`. 

Note you can avoid the above by `+`-ing your theme on to the plot, and adding a `*_limits = c(NA, NA)` argument (or `*_limits = c(0, NA)` for bars) in the `gg_*` function. 
