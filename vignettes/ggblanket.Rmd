...
The pal is _generally_ inherited to subsequent geom layers regardless of whether there is a `col` aesthetic.
...
For _all_ plots with multiple geom's, you will need to think about if there is any desired drawing order you want, and if so choose the `gg_*` function of the layer you would like on the bottom.

For plots where some geoms have a col aesthetic and some do not, you must choose a `gg_*` function that relates one of the layers with a col aesthetic. Or, if this conficts the desired drawing order, then choose `gg_blank`. The reason for this is as alluded to earlier, of how ggblanket under the hood creates a dummy col scale for when there is no `col` aesthetic. 
...
`gg_*` functions do not directly support aesthetics of `alpha`, `size`, `linewidth`, `shape`, and `linetype`. However, you could use `gg_blank` in addition to a `col` aesthetic, and then add the title of your new aesthetic to align with the `col_title`.
