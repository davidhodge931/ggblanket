...
If the `gg_*` guesses an incorrect `aspect`, then the user can change this. The user can set these side-effects in `set_blanket` by setting `axis_line_transparent`, `axis_ticks_transparent` or , `panel_grid_transparent` to `FALSE`.

Any `theme` used in `set_blanket` should be designed to anticipate the side-effects desired (i.e. include panel grid, axis line and axis ticks in both directions). 
