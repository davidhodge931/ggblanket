...
15. Consistent effortless bordered style 
...
Other aeesthetics are available: `alpha`, `size`, `shape`, `linetype` and `linewidth`. These can be either mapped as an aesthetic, or set to a value. The `mapping` argument is also provided for use with `ggplot2::after_stat`. 
...
### 11.  Side-effects to the `theme` based on the `aspect`

The `gg_*` function adds helpful side-effects to the theme based on the `aspect`, `aspect_axis_line_rm`, `aspect_axis_ticks_rm` and , `aspect_panel_grid_rm` arguments. 

* Where `aspect = "x"`, the `gg_*` function will remove the y axis line/ticks and the x gridlines from the `theme` (by changing their colour to "transparent"). 
* Where `aspect = "y"`, the opposite will occur. 

If the `gg_*` guesses an incorrect `aspect`, then the user can change this. The user can change any of these side-effects by setting `axis_line_transparent`, `axis_ticks_transparent` or , `panel_grid_transparent` to `FALSE`.
...
Certain geoms are classified as "bordered" (e.g. bar, area, tile, polygon etc). Point geoms with shapes 21-25 are also considered bordered. This classification determines whether border logic applies to the underlying colour/fill and equivalent palettes for these aesthethics. By default bordered polygons are applied a multiply blending of their colour to itself, and fill is left unchanged. Use `bordered = FALSE` to turn this off, or `set_blanket(bordered_colour_by = \(x) x, bordered_fill_by = \(x) x, ...)`. It is intended that in `set_blanket`, users rely on the `col` argument primarily to colour/fill, and likewise the `col_palette_*` palettes. The `colour`/`fill` and `colour_palette`/`fill_palette` arguments are provided in `gg_blanket` to support deviations from this approach. 

```{r}
set_blanket(
  theme = theme_lighter(),
  bordered_colour_by = \(x) scales::col_darker(x, amount = 50),
