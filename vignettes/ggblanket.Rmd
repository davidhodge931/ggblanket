...
set_blanket(
  theme = theme_lighter(
    panel_grid_colour = "orange", #for emphasis
  )
)
p1 <- palmerpenguins::penguins |>
  drop_na() |>
...
A `symmetric` continuous scale can often be useful and look good. 

This is the term used to describe a continuous scale where the limits equals the range of the breaks, and `*_expand = c(0, 0)`.

By default, the `gg_*` function guesses the symmetric scale based on the `"aspect"` of the plot. 

Use `symmetric = "none"` to revert to the normal limits and expand defaults.
