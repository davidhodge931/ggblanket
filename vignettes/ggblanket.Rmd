...
penguins |> 
  ...
  gg_density(
    x = flipper_length_mm,
    col = species,
    col_pal = c("#ff483a", "#16607a", "#6d4420"),
  )
```
...
If you do _not_ want the `gg_*` function to make any changes to your provided mode, you should instead `+` the mode on  (e.g. `+ grey_mode_rt()`). 

So, other themes generally should be `+`-ed onto the plot. 
...
Three mode families are provided: `grey_mode_*`, `light_mode_*` and `dark_mode_*`.

Each mode family provides 5 variants that differ based on legend placement, which is represented by the suffix of the mode name of `rt`  (right-top), `r` (right-centre), `b` (bottom), `t` (top) and `n` (none). 

The default is `grey_mode_rt`.

These are designed for use with the `mode` argument, and the automatic gridline/axis-line/ticks removal that comes with that argument.

```{r, fig.asp=0.7}
# mode_set(grey_mode_t())
