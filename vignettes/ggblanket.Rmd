...
15. Supports `ggiraph` interactivity with `gg_blank` and `add_tooltip`
...
ggblanket supports `ggiraph` interactivity with `gg_blank` and `add_tooltip` functions. Users could use a similar approach with `plotly::ggplotly`, if they prefer. 
...
girafe(
  ...
  width_svg = 5,
  height_svg = 4,
  options = list(opts_hover(css = "fill:red; stroke: red"))
)
```
