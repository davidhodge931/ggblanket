...
Commonly used aesthetics can be added directly as arguments. 
...
(Note: you can use `set_blanket(geom_colour = lightness[1], ...)` to make legend symbols black where needed). 
...
The `gg_*` function will guess the orientation of the plot. 

Where it guesses `x_orientation = TRUE` and there is a continuous y scale, it will make this y scale 'symmetric' if possible. This is that the `y_limits` will be the range of the `y_breaks` and `y_expand` will be `c(0, 0)`. The vice-versa will occur where it guesses `y_orientation = TRUE`.
...
(Note this 'symmetric' scale does not occur where the associated `*_limits` are specified or the `*_transform` is non-linear).
...
Aesthetics directly added in the `gg_*` function are globally inherited, whereas those added via the `mapping` argument are only available locally to the layer associated with the `gg_*` function.  

It generally works best to add as many of the globally inherited aesthetics as possible to the `gg_*` function. The `gg_*()` function should also be appropriate to be the bottom geom of the plot, given that geoms are drawn in order. Subsequently, you may sometimes need to use `gg_blanket()` which defaults to a `blank` geom.
...
These are built for use with the `mode` argument in the `gg_*` functions, which has helpful side-effects. 

The `gg_*` function will guess the orientation of the plot. Where it guesses `x_orientation = TRUE`, it will remove the y axis line/ticks and the x gridlines from the `mode`. The vice versa will occur where it guesses `y_orientation = TRUE`.

If you want to use a theme with _no_ side-effects, then `+` on the theme instead.
