...
7. Default x/y/col titles converted with to sentence case
8. Ability to set the theme 
9. Light_mode and dark_mode theme functions
10. Pretty defaults for symmetry
11. Access to other geom_* arguments via ...
12. Spatial data visualisation
13. Ability to add multiple geom_* layers
14. A framework able to be extended further
...
The `pal` argument is used to customise the colours of the geom. A user can provide a vector of colours to this argument. It can be named or not. It works in a consistent way - regardless of whether a `col` argument is added or not. 
...
### 7. Default x/y/col titles converted to sentence case

Default x/y/col titles are converted to sentence case with snakecase::to_sentence. All titles can be manually changed using the `*_title` arguments. However, with the default conversion, it is intended that titles are less likely to need to be manually changed than if default titles were left as variable names.
...
A `theme` argument is available to set the theme on a plot. By using the `theme` argument, the theme provided will control all theme aspects, except (1) the placement of the legend and (2) gridline removal. Note gridlines can be adjusted within the `gg_*` function, if necessary. If users want their theme to adjust _everything_, then they can `+` their theme as a layer instead.

Users can set the theme globally as well using `ggplot2::theme_set`. This will set all subsequent plots to use this theme by default - unless the theme argument in a `gg_*` plot is changed. The hack of how this works under the hood means that if you wish to set the theme to `ggplot2::theme_grey` with the default `base_size` of 11, then you must it adjust it slightly (e.g. `theme_set(theme_grey(11.01)))`
...
### 9. Light and dark mode theme functions
...
### 13. A framework able to be extended further
