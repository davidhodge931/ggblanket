...
9. A theme_light_mode and a theme_dark_mode function
...
Light and dark theme functions are provided to help users create a quick clean looking theme: `theme_light_mode` (the default) and `theme_dark_mode`. These have arguments to change the family, face, colour, size, margins and vertical adjustment of the title, subtitle, caption and base text.

The theme can be set globally using `ggplot2::theme_set`. If you wish to set the theme to `ggplot2::theme_grey` with the default `base_size` of 11, then you must it tweak it ever so slightly (e.g. `theme_set(theme_grey(11.01)))`
