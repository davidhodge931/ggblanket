...
These are built for use with the `mode` argument in the `gg_*` functions, which has helpful side-effects for the x and y orientations (i.e. removing particular axis lines/ticks and gridlines).

If you want to use a theme with no side-effects, just + the theme on to the plot.
...
The key thing to know is that you can easily turn this 'symmetric' scale off using `*_limits = c(NA, NA)` (or `*_limits = c(0, NA)` for bars etc). Use `lubridate::NA_Date_` instead of `NA` for a date variable.
...
Generally, it works best to add all aesthetics required for the plot to the `gg_*` function. 
