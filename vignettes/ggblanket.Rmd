...
The scales are built within the `gg_*()` function _without_ knowledge of later geom layers. So, in some situations, users will need to take care to accommodate this within the `gg_*()` function. Note the `gg_*()` function will build scales only with regard to the relevant aesthetics to the associated geom (e.g. `geom_col()` does not understand the `xmax` aesthetic).
