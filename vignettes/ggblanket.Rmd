...
* to remove a `x_label` or `y_label`, use `+ labs(... = NULL)` 
* to remove a `col_label`, use `+ labs(colour = NULL, fill = NULL)` or `+ theme(legend.title = element_blank()`
* for a quick non-bold title, use `subtitle = "\n..."`.
...
The key thing to know is that you can easily turn this 'symmetric' scale off using `*_limits = c(NA, NA)` (or `*_limits = c(0, NA)` for bars etc). For a date variable, use `lubridate::NA_Date_` instead of `NA`.
