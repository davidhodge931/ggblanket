...
ggblanket keeps unused factor levels in the plot. If users wish to drop unused levels they should likewise do it in the data prior to plotting. E.g. `data |> mutate(var = forcats::fct_drop(var)) |> gg_*()`. 

##### Stats locked except in gg_blank

ggblanket locks the `stat` of each `gg_*` function to the default of the respective geom, with the exception of `gg_blank`. The `gg_blank` function has a `stat` argument that users can change. This means if you would like to use a stat different to the default for a `gg_*` function, you need to use `gg_blank`. Setting the `stat` to that of the layer to follow can help ensure scales are created correctly. Note `gg_blank` only accepts stats that are characters.
