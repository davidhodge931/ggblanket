...
### 9. Three gg_theme functions to create a quick theme 

Three theme functions are provided to help users create a quick clean looking theme: `gg_theme_light` (the default), `gg_theme_dark` and `gg_theme_void`. 

Element prefixes to modify are: `text_*`, `title_*`, `subtitle_*`, `caption_*`, `background_pal_*`, `line_*` (i.e. the axis line), `ticks_*`, and `gridlines_*`.

If you wish to use these theme functions outside of a ggblanket visualisation function, remember to remove the appropriate gridlines with `theme(panel.grid.major.y = element_blank())`. 
...
penguins |>
  gg_point(
    ...
    subtitle = "Palmer Archipelago, Antarctica",
    caption = "Source: Gorman, 2020",
    theme = gg_theme_dark(text_size = 10))
```
...
penguins |>
  gg_point(
    ...
    subtitle = "Palmer Archipelago, Antarctica",
    caption = "Source: Gorman, 2020",
    theme = gg_theme_void(text_size = 10))
```
