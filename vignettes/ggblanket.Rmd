...
For geom's that have interior and outline colour, you can turn off one of these as follows:

a). For plots with _no_ `col` aesthetic, use `alpha_pal = 0` or `col_pal = scales::alpha(..., alpha = 0)`
b). For plots with a `col` aesthetic, either use the above approach with `col_pal_na = scales::alpha(..., alpha = 0)`too - or just use `fill = NA`/`colour = NA`.
