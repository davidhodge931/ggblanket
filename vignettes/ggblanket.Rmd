...
8.  Provision of families of themes
9.  Pretty continuous x and y scales with easy opt-out
10.  Pretty gridlines removal
...
Whenever a `facet` variable is provided (and a `facet2` variable _not_ provided), the `facet_layout` will default to `"wrap"` - but this can be changed to `grid`. 
...
Whenever a `facet2` variable is provided, the `facet_layout` will default to `"grid"`, but this can be changed to `wrap`. 
...
ggblanket provides a pretty continuous y scale for non-horizontal plots that has: 
...
(Note this pretty scale does not occur where the scale has a transformation that is _not_ `"identity"`, `"reverse"`, `"date"`, `"time"` or `"hms"` - or where the other positional scale is binned.) 
