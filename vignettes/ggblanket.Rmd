...
There are also other more minor and unusual things that you cannot do.

For aesthetics of alpha, size, linewidth, and linetype, use `gg_blank` and change the title of the aesthetic you've added to align with the title of the col aesthetic.

```{r}
penguins |> 
  gg_blank(x = flipper_length_mm, y = body_mass_g, col = species) +
  geom_point(aes(x = flipper_length_mm, y = body_mass_g, col = species, shape = species)) +
  labs(shape = "Species")
```
...
For some other minor ggplot2 functionality that is unsupported by ggblanket, you can just add a ggplot2 layer to your ggblanket code. For example, using `+ labs(x = NULL)` enables the space to work better than `x_title = ""`.
