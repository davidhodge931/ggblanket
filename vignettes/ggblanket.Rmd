...
This classification determines whether border logic applies to the underlying colour/fill and equivalent palettes for these aesthetics. By default border polygons are applied a `col_multiply` blend, and fill is left unchanged. 
...
The `blank` geom gets `border = TRUE` unless otherwise specified. As such, border colouring/filling only passes through to `+ geom_` layers for mapped `col` aesthetics. Where `col` is not mapped, use `col_multiply` and `col_screen` in subsequent layers to replicate the default ggblanket look.
...
palmerpenguins::penguins |>
  ...
  annotate("rect", 
           ...
           colour = NA,
           fill = jumble[9]) +
  geom_point(colour = col_multiply(get_geom_defaults("point")$colour))
```
