...
14.  A way to create powerful custom plot functions
...
By using the `theme` argument, your theme will control all theme aspects, _other_ than the legend position, direction and justification. These are controlled within the `gg_*` function, so that legend positions can be adjusted easily with the `col_legend_place` argument (e.g. ``col_legend_place = "r"`). If you prefer to have your `theme` adjust everything _including_ the legend, then add your theme as a layer instead (e.g. `+ theme_grey()`). 

Note, the default theme within `gg_*` guesses the gridlines you might want based on the class of `x` and `y` variables in the data. 

If you provide a theme, it will add gridlines, as per that theme. You may want to create two versions of your custom theme: one with horizontal gridlines and one with vertical gridlines.
...
The `gg_theme` function allows you to create a quick simple theme. 

It includes arguments for adjusting gridlines, background colours, text, axis lines and ticks. 
...
Note that ggblanket keeps `x` and `y` out-of-bound values (i.e. it uses scales::oob_keep under the hood). However, you can zoom in using `coord = coord_cartesian(xlim = ..., ylim = ...)` (or by filtering the data).
