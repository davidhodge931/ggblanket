...
* updating a series of geom defaults (not commononly used for annotation)
* updating a series of geom defaults commonly used for annotation (i.e. `*_vline`, `*_hline`, `*_abline`, `*_curve`, `*_text` and `*_label`).

Within the `set_blanket` function, the user can select the `mode`, and the `geom_colour` and `annotate_colour` with the fill inheriting from these.
