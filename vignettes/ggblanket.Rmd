...
These functions were built for use within `set_blanket`, as then `border_colour_transform` and `border_fill_transform` adjustments can apply based on these (e.g. if the panel is dark). 
...
Certain geoms are classified as being with a border (e.g. bar, area, tile, polygon etc). Point geoms with shapes 21-25 are also considered with a border. 

This classification determines whether border logic applies to the underlying colour/fill and equivalent palettes for these aesthetics. By default border polygons are applied a `blend_multiply` blend, and fill is left unchanged. 

Use `border = FALSE` to turn this off within the `gg_*` function (or globally with `set_blanket(border_colour_transform = \(x) x, border_fill_transform = \(x) x, ...)`).

```{r}
set_blanket(
  theme = theme_lighter(),
  border_colour_transform = \(x) scales::col_darker(x, amount = 40),
...
The `blank` geom gets `border = TRUE` unless otherwise specified. As such, border colouring/filling only passes through to `+ geom_` layers for mapped `col` aesthetics. Where `col` is not mapped, use `blend_multiply` and `blend_screen` in subsequent layers to replicate the default ggblanket look.
