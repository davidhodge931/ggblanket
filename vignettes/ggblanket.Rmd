...
10. Ability to set the mode globally
11.  A symmetrical continuous scale with easy opt-out
12. Access to other `geom_*` arguments via `...`
13. Turn-off either coloured outlines or interiors
14. Ability to add multiple `geom_*` layers
15. Other key differences to ggplot2
...
To avoid these side-effects, `+` your theme on as a layer.
...
Each theme family provides 5 variants that differ based on legend placement, which is represented by the suffix of the mode name of `rt`  (right-top), `r` (right-centre), `b` (bottom), `t` (top) and `n` (none). 

The default is `grey_mode_rt`.

These are designed for use with the `mode` argument.
