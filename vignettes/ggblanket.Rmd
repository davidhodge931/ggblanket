...
The `blank` geom gets `bordered = TRUE` unless otherwise specified. As such, bordered colouring/filling only passes through to `+ geom_` layers for mapped `col` aesthetics. Where `col` is not mapped, use `col_multiply` and `col_screen` in subsequent layers to replicate the default ggblanket look.
