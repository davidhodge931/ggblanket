...
However, in other situations, users will need to take care to ensure the `gg_*()` function is (a) relevant to a `geom_*()` that can _understand_ the aesthetics and (b) includes other arguments needed to construct the scales correctly. The `gg_blanket()` function with it's `"blank"` geom can be useful here. 
