...
The colour and fill aesthetics of ggplot2 are merged into a single concept represented by the `ink` argument. 

This combined aesthetic means that all colour outlines and all fill interiors should be coloured with the `ink_palette` by the `ink` variable. 
...
penguins2 |>
  gg_jitter(
    ...
    ink_label = "Bill\nlength (mm)",
    ink_steps = TRUE,
    ink_breaks = \(x) quantile(x, seq(0, 1, 0.25)),
    ink_palette = viridis::rocket(n = 9, direction = -1),
  )
```

### 8.  Smart `*_label` defaults for axis and legend titles

The `x_label`, `y_label` and `ink_label` for the axis and legend titles can be manually specified with the applicable `*_label` argument (or `+ ggplot2::labs(...)`).
...
penguins2 |>
  gg_smooth(
    ...
    y = body_mass_g,
    ink = sex, 
    ink_palette = c("#003F5CFF", "#FFA600FF"),
    colour = "#BC5090FF", 
    linewidth = 1, 
    linetype = "dashed",
    alpha = 1, 
