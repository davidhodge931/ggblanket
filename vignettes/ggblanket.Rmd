...
If you do not actually want a col aesthetic in your bottom plot layer, but do want it in subsequent layers- then you should  use the `gg_blank` function. The subsequent layer without the col aesthetic can use `inherit.aes = FALSE` argument. 

Note the `gg_*` function builds the x and y scales based on the `data`, `stat`, `x`, and `y` in the `gg_*` function - but these can be adjusted using the `facet_scales`, `*_include` and `*_limits` arguments. The plot scales are constructed without knowledge of subsequent layers. 
...
penguins |>
  ...
  gg_blank(
    ...
    y_include = 0,
    width = 0.75) +
  geom_col(aes(x = sex, y = body_mass_g), fill = "#d3d3d3", inherit.aes = FALSE) +
