...
Almost every geom in ggplot2 is wrapped. So there are almost 40 of them!
...
These are built for use with the `mode` argument in the `gg_*` functions, which has helpful side-effects for x and y orientations (i.e. removing particular axis lines/ticks and gridlines). If you want to use a theme with no side-effects, + the theme to the plot.
...
However, the vice-versa will occur where a y orientation is guessed with a 'symmetric' continuous x scale.
...
This 'symmetric' scale does not occur where the scale has a non-linear `*_transform` - or where the other positional scale is binned. 

Sometimes you may need to add `*_expand_limits = 0`, if the lower bound is close to 0.
