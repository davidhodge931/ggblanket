...
There are also other somewhat unusual things that you cannot do.

For aesthetics of alpha, size, linewidth, and linetype, use `gg_blank` with a col argument and add a title of the aesthetic you've added to align with the col_title.
...
For situations related to unsupported computed variables, use ggplot2. You can mimic the ggblanket look and feel if you would like using gg_theme and other scale_* modifications.
...
For some other minor ggplot2 functionality that is unsupported by ggblanket, you can just add a ggplot2 layer to your ggblanket code. 

For example, using `+ labs(x = NULL)` enables the space to work better than `x_title = ""`.

```{r}
penguins |>
  gg_bar(species, width = 0.75) +
  labs(x = NULL)
```

Likewise, for example, users could access the unsupported `switch` argument in `ggplot2::facet_grid` by adding this as a layer.  
