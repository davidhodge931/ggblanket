...
However, in other situations, users will need to take care to ensure the `gg_*()` function is (a) relevant to a `geom_*()` that can _understand_ the aesthetics required and (b) includes all arguments needed to construct the scales correctly. The `gg_blanket()` function with it's `"blank"` default geom can be useful here. 
