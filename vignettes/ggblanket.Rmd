...
Each `gg_*` function wraps a `ggplot2::ggplot()` function with the associated `ggplot2::geom_*()` function. Almost every geom in ggplot2 is wrapped. Most aesthetics can be added as arguments. 
...
Otherwise, they will convert the variable name to a label name using the `label_to_case` function, which defaults to sentence case (i.e. `snakecase::to_sentence_case`). 
...
This is why you can turn off  _either_ colour or fill using `colour = NA`/`fill = NA`, as previously mentioned.
...
These are built for use with the `mode` argument in the `gg_*` functions, which has  side-effects for x and y orientations (i.e. removing particular axis lines/ticks and gridlines). If you want to use a theme with no side-effects, then `+` the theme to the plot.

Note to remove a `*_label`, use `+ labs(... = NULL)` or `+ theme(legend.title = element_blank()`.
...
The `gg_*` function will generally create a 'symmetric' continuous y scale with: 
...
Note this 'symmetric' scale does not occur where the scale has a non-linear `*_transform` - or where the other positional scale is binned. Also, sometimes you may need to add `*_expand_limits = 0`, if the lower bound is close to 0.
