...
8.  `grey_mode_*`, `light_mode_*` and `dark_mode_*` and theme families
9.  Access to other `geom_*` arguments via `...`
10. Pretty scales, and automatic gridline and axis line/ticks removal
11. Turn-off either colour or fill
...
Each `gg_*` function wraps a ggplot2 `ggplot2::ggplot()` function with the applicable ggplot2 `geom_*()` function. Each `gg_*` function is named after the `geom_*` function they wrap. 
...
It is intended that this `theme` argument is used in combination with the `*_mode_*` themes. You can also set these globally using `ggplot2::theme_set()`. 

If you would like to provide your own theme, you should `+` the `theme` on as a layer, as the `gg_*` function removes selected gridlines, axis lines and ticks.
...
* which scale to make symmetric 
* which gridlines to remove 
* which axis line and ticks to remove
...
This symmetric scale can be turned off easily using `y_limits = c(NA, NA)` (or `y_limits = c(0, NA)` for bars etc).
...
If you want different gridlines or axis lines/ticks to that which is defaulted, you should `+ *_mode_*` instead.  
...
For geom's that have both interior and outline colour, you can turn off one of these.

Use `alpha_pal = 0` for the interior fill, or `col_pal = scales::alpha(..., alpha = 0)` for the outline colour.

For the outlines with a `col` aesthetic, you might need to modify the `col_pal_na` value using `col_pal_na = scales::alpha("darkgrey", alpha = 0)`. 

[Note where there is a `col` aesthetic, another way is to use `fill = NA`/`colour = NA`.]
...
The `gg_*` function puts the aesthetic variables (i.e. `x`, `y`, `col`) within the wrapped `ggplot` function. Therefore, these aesthetics will inherit to any subsequent layers added - unless a `inherit.aes = FALSE` argument is added. `facet` and `facet2` will likewise inherit - as will `data`, `coord` and `theme`.
