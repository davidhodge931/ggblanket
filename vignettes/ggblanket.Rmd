...
12. Arguments to customise setup with `set_blanket()`
...
Users can make plots with multiple `ggplot2::geom_*` layers with consideration that:

* the `gg_*()` geom layer will be the bottom geom layer of the plot, and subsequent `geom_*()` layers are placed on top. 
* scales are built within `gg_*()` and _without_ reference to later geom layers.

Sometimes this will all just work without much thought. 
...
However, in other situations, users will need to take further care to ensure the `gg_*()` function is (a) relevant to a `geom_*()` that can _understand_ the aesthetics required and (b) includes all arguments needed to construct the scales correctly. The `gg_blanket()` function with it's `"blank"` default geom can often be useful here. 

(Note, aesthetics added to the `mapping` argument are only available _locally_ to the geom layer associated with the `gg_*` function). 
