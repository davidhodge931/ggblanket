...
8.  `*_title` defaults to `snakecase::to_sentence_case`
9.  Other `ggplot2::geom_*` arguments via `...`
10. `theme_greyer`, `theme_lighter`, `theme_darker` and `theme_beiger` themes 
11. Set and forget with `set_blanket()`
12. An `aspect` for vertical/horizontal ease 
13. A `*_limits_to_breaks` continuous scale
14. `bordered` style ease
15. An `annotate` argument to plot annotations underneath
16. A `gg_blanket()` function with `geom` flexibility
...
The `*_title` for the axis and legend titles default to `snakecase::to_sentence_case` (or the label attribute). Otherwise, they take on any value provided by the user, including `NULL` which removes the title.
...
ggblanket provides `theme_greyer`, `theme_lighter`, `theme_darker` and `theme_beiger` theme functions. 

Key arguments to customise these are `text_size`, `text_colour`, `panel_background_fill`, `plot_background_fill` and `panel_grid_colour`.

Note when using these themes: 

* When both `title` and `subtitle` are added to the `gg_*` function, add `" \n"` to `subtitle`
* When only `title`  is added, add `" \n"` to `title`. 

If the theme has been converted to marquee richtext, then use `" <br>"` instead of `" \n"`.
...
The `aspect` parameter describes the intended way that a person would look at a plot - and then applies the `axis_line_aspect`, `axis_ticks_aspect` and `panel_grid_aspect` behaviour to the plot.
...
The default `*_limits_to_breaks`is largely determined by the `aspect` of the plot. Where `aspect = 'x'`, then by default `x_limits_to_breaks = FALSE` and `y_limits_to_breaks = TRUE`. For `aspect = 'y'`, the vice versa occurs.
...
Certain geoms are classified as being with a bordered (e.g. bar, area, tile, polygon etc). Point geoms with shapes 21-25 are also considered with a bordered. 

This classification determines whether bordered logic applies to the underlying colour/fill and equivalent palettes for these aesthetics. By default bordered geoms on light backgrounds are applied a `blend_multiply` blend, and fill is left unchanged. On dark backgrounds, the `blend_screen` function is applied.  

Use `bordered = FALSE` to turn this off within the `gg_*` function (or globally with `set_blanket(bordered_colour_transform = \(x) x, bordered_fill_transform = \(x) x, ...)`).

```{r}
set_blanket(
  theme = theme_lighter(),
  bordered_colour_transform = \(x) scales::col_darker(x, amount = 40),
...
An annotate argument is provided to enable annotations under the
 `gg_*` geom.
...
palmerpenguins::penguins |>
  ...
  gg_point(
    ...
    size = bill_length_mm,
    shape = sex,
    annotate = annotate_panel_shade(xmin = I(0.9)),
  ) 
```
...
This function can often be used where a geom is needed to be plotted that is not provided as a `gg_*` function.

The `blank` geom gets `bordered = FALSE` unless otherwise specified. Use `bordered = TRUE` where there is polygons in later layers. Where `col` is not mapped, use `blend_multiply` and `blend_screen` as needed in subsequent layers.
