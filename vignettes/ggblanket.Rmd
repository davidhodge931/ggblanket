...
The `gg_*` function builds the x and y scales based on the `data`, `stat`, and `x` and `y` variables in the `gg_*` function. It does not know about any subsequent layers that are added. You use `*_include`, `*_limits` and `stat` to adjust the pre-built scales to accommodate subsequent layers.
