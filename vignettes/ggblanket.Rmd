...
8. Access to other `geom_*` arguments via `...`
9. A symmetrical continuous scale with easy opt-out
10. Ability to add multiple `geom_*` layers
11. Access other aesthetics via `mapping`
12.  A `mode` argument designed for provided `*_mode_*` themes
13.  Families of `*_mode_*` themes that differ by legend placement 
14. Set the mode and geom defaults with `set_blanket`
...
Subsequently, for geoms that have _both_ coloured outlines and interiors, you can turn off  _either_ of these using `colour = NA`/`fill = NA` - or fix one to a single colour with `colour = "black"`/`fill = "lightgrey"` etc.
...
penguins |>
  ...
  gg_smooth(
    ...
    y = body_mass_g,
    col = sex, 
    colour = "#112E51", # via ... from geom_smooth
...
### 9. A symmetric continuous scale with easy opt-out
...
The defaults `set_blanket()` work well. 

But you use this to set a different default theme to the `mode` argument, and a different default geom colour for outline/fill - or a different annotation colour (i.e. the colour used for `*_vline`, `*_hline`, `*_abline`, `*_curve`, `*_text` and `*_label`). You can use the `mode` and `geom_defaults` arguments in `set_blanket` to do this. 
