...
14. Colour-blind safe and accessible defaults
...
Note this 'symmetric' scale does not occur where `*_limits` are specified or the `*_transform` is non-linear.
...
The default colour/fill scale palettes (i.e. `col_palette_d` and `col_palette_c`) are relatively colour-blind safe.

They are also relatively accessible for themes with light backgrounds.

```{r}
#jumble default continuous colour scale palette (and default NA colour)
colorspace::swatchplot(c(jumble, grey), cvd = TRUE)
```

```{r}
#viridisLite::mako default continuous colour scale palette (and default NA colour)
