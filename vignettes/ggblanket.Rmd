...
However, each title can be manually changed using the applicable `*_label` argument. 

The default `snakecase::to_sentence` conversion can be turned off using `titles_to_case = \(x) x` or changed (e.g. `snakecase::to_label_case`). 
