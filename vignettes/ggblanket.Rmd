...
3.  The `col` aesthetic is highjacked to be both colour and fill
...
In ggblanket, the `col` aesthetic is highjacked to instead represent *both* the `colour` *and* the `fill` aesthetic.
...
Likewise, when you add `col = "red"`, the red is added to the `colour` and the `fill`.
...
Note the `theme` argument in `set_blanket` can also be used to default a `*_label` to `NULL` (e.g. `set_blanket(theme = list(theme_lighter(), labs(colour = NULL, fill = NULL)))`).
