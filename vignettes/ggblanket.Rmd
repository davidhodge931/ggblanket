...
For some ggplot2 functionality unsupported by ggblanket, you can add the applicable ggplot2 layer to your ggblanket code. For example, using `+ labs(x = NULL)` or `+ facet_grid(..., switch = "y")`.
...
penguins |>
  ...
  facet_grid(cols = vars(species),
             rows = vars(island), 
             switch = "y") +
  theme(strip.switch.pad.grid = unit(3, "mm")) +
  labs(x = NULL)
```

Some functionality is just not supported in ggblanket. For example, you cannot use _computed_ variables. Instead you will need to use  ggplot2, if these are required.
