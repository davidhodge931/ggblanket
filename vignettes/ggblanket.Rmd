...
* the geom defaults, including the colour (and fill) of geoms (as per `weave_geom_defaults`)
* the colour (and fill) palettes for discrete, continuous and ordinal (as per `weave_col_palette`)
* the theme, and how/what side-effects are to be applied (as per `weave_theme`
* the function to apply to a unspecified/unlabelled `x_label`, `y_label`, `col_label` etc (as per `weave_label_case`.

In addition to `set_blanket`, `set_font_defaults()` and `set_reference_defaults()` can be used to customise the "text", "label", "abline, "vline" and "hline" geom defaults.
...
set_blanket(
  theme = rlang::list2(
    ...
    labs(colour = NULL, fill = NULL, linetype = NULL, shape = NULL, alpha = NULL)
  ),
  colour = "tan", 
  col_palette_d = c("#003f5c", "#bc5090", "#ffa600", "#357BA2"),
  col_palette_na_d = "#78909C",
  col_palette_c = viridisLite::rocket(n = 9),
)

set_font_defaults(text_size = 9 / 2.835)

set_reference_defaults(colour = "#78909C", linewidth = 0.25)
...
p1 <- penguins2 |>
  ...
  annotate("text", x = I(0.2), y = I(0.8), label = "Here")
...
p2 <- penguins2 |> 
  ...
  geom_vline(xintercept = 200) +
  annotate("label", x = I(0.8), y = I(0.8), label = "Here")
