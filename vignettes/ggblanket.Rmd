...
13. A `symmetric` continuous scale
14. A `bordered` style 
...
A `symmetric` continuous scale can often be useful and look good. It refers to the axis line and ticks appearing symmetrical where (1) the limits equals the range of the breaks, and (2) `*_expand = c(0, 0)`.

By default, the `gg_*` function determines the symmetric scale based on the `aspect` of the plot. This can be adjusted by the user using the symmetric argument, including using `symmetric = NA` to revert to the normal limits and expand defaults.
...
economics |>
  ...
  annotate("rect", 
           xmin = lubridate::ymd("2010-12-31"), 
           xmax = lubridate::ymd("2020-12-31"),
           ymin = -Inf, 
           ymax = Inf,
           colour = NA,
           fill = jumble[9]) +
