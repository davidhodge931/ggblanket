...
17. 32 different geoms wrapped into a `gg_*` function! 
...
`gg_*` functions do not directly support aesthetics of `alpha`, `size`, `linewidth`, `shape`, and `linetype`. However, you can use `gg_blank` in addition to a `col` argument, and then add the title of your new aesthetic to align with the `col_title`.
...
Use `colour = NA` or `fill = NA` to turn off one of the aesthetics that are part of the ggblanket col aesthetic. You can also use `gg_blank` to work with many other extension geom's that have a stat of "identity" or similar. 
