...
The plot scales are constructed without knowledge of subsequent layers. The `gg_*` function builds the x and y scales based on the `data`, `x`, and `y` in the `gg_*` function. These scales can be adjusted to accommodate subsequent layers using the `*_include` or `*_limits` arguments. Note for `gg_blank`, the scale is also built based on the `stat`. 
...
For all `gg_*` functions other than `gg_blank`, the stat is locked to the default as per the relevant `ggplot2::geom_*` function. This means if you would like to use a stat different to the default, you should use `gg_blank`. You can also set the `stat` to that of the layer to follow if necessary to help the scales etc be created appropriately. Note `gg_blank` only accepts stats that are characters.
...
penguins |>
  ...
  geom_pointrange(stat = "summary", col = pal_blue)
```
...
These deviations from ggplot2 were considered to be safer defaults for ggblanket (i.e. fail by plotting data outside the panel rather than fail by dropping data).
...
ggblanket does not drop unused any factor levels from the plot. If users wish to drop unused levels they should do by mutating the variable prior to plotting (i.e. `data |> mutate(var = forcats::fct_drop(var))`). Similarly, ggblanket requires unquoted variables for it's aesthetics. If you wish to reorder a variable or drop NAs, you should do it likewise prior to plotting. 
