...
8.  A `theme` argument designed for provided `*_mode_*` themes
...
A `theme` argument has been designed for use with the `*_mode_*` themes. 

When a theme is provided it, the `gg_*` function will:

* determine whether the plot is `flipped` or not
...
If you do _not_ want the `gg_*` function to make any changes to your provided theme, you should instead `+` your theme on as a layer  (e.g. `+ grey_mode_rt()`).

The `theme` argument can also be accessed globally using using  `ggplot2::theme_set()`.  

Note, if you would like to use a plain title, a convenient hack is to use the `subtitle` argument with a `"\n"` at the start of the character string. 
...
Three theme families are provided: `grey_mode_*`, `light_mode_*` and `dark_mode_*`.
...
These are designed for use with the `theme` argument.
...
### 9. A symmetrical continuous scale with easy opt-out
...
### 11. Turn-off either coloured outlines or filled interiors
