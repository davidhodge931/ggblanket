...
17. Works with the `ggtext` package

### 1. `gg_*` wrapper functions to plot a single geom

ggblanket provides over 30 `gg_*` functions. This is the majority of the geoms in ggplot2.  
...
### Making _part_ of the col aesthetic fixed or off

In ggblanket, the `col` aesthetic refers to both the ggplot2 colour and fill aesthetics. If you wish to turn off one of these, or set it to a fixed colour, you can via the ... arguments which access the geom_* function. Use `colour = NA` or `fill = NA` to turn off part of the `col` aesthetic, or `colour = "red"` or `fill = "red"` to fix part of the colour aesthetic to a colour.
...
### Other non-supported geoms

You may or may not be able to work with other geom's by using `gg_blank`, depending on what the stat is in the function. If the stat is "identity" or another stat used by ggblanket, it will work `gg_blank` and that stat referenced. For other stat's, it depends on the nature of the stat.  
