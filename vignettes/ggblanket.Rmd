...
If both `facet`and `facet2` variables are provided, then the gg_* function will facet in a grid of `facet` by `facet2`.

If only one facet (or facet2) variable is provided, it will default to using a facet layout of `"wrap"`. The user can specify `facet_layout = "grid"`, if they wish to access the proportional facet sizes enabled by the `facet_space` argument. 
