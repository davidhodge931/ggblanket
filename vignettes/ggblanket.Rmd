...
To make a plot with multiple layers, users should consider:

* that the `gg_*()` geom layer will be the bottom geom layer of the plot, and subsequent geom layers will plot on top. 
* that scales are built within `gg_*()` and without reference to later geom layers. Therefore, the `gg_*()` function should be relevant to a `geom_*()` that can understand the aesthetics and other arguments needed to construct the scales correctly (and/or include appropriate `*_expand_limits` arguments). 
