...
10. `theme_lighter`, `theme_darker` and `theme_whiter` themes 
...
ggblanket provides `theme_lighter`, `theme_darker` and `theme_whiter` theme functions. 

The {flexoki} package can be used to create nice coloured themes. See the help for `theme_lighter` and `theme_darker`.
...
This function can often be used (1) where a annotation is needed to be plotted first, or (2) where a geom is needed to be plotted that is not provided as a `gg_*` function.

The `blank` geom gets `bordered = TRUE` unless otherwise specified. As such, bordered colouring/filling only passes through to `+ geom_` layers for mapped `col` aesthetics. Where `col` is not mapped, use `blend_multiply` and `blend_screen` in subsequent layers to replicate the default ggblanket look.
...
palmerpenguins::penguins |>
  ...
  annotate_shade(xmin = I(0.9)) +
  geom_point(colour = blend_multiply(get_geom_defaults("point")$colour))
```
