...
For plots with _no_ `col` aesthetic, use `alpha_pal = 0` or `col_pal = scales::alpha(..., alpha = 0)`.

For plots with a `col` aesthetic, either use the above approach with `col_pal_na = scales::alpha("darkgrey", alpha = 0)` too (or use `fill = NA`/`colour = NA` as a shortcut).
