...
`light_mode_*`, `grey_mode_*` and `dark_mode_*` theme families are provided with variants that differ based on legend placement with suffix `r` (right), `b` (bottom), and `t` (top). 
...
If it is guessed an x orientation, it will:

* remove the y axis line/ticks and the x gridlines from the `mode` theme. 
* generally make _linear_ y continuous scales 'symmetric' with (a) `y_limits` the range of `y_breaks` and (b) `y_expand = c(0, 0)`.

The vice versa occurs when it guesses a y orientation. 

To avoid these side-effects:

* `+` the theme on to the `gg_*` output
* Change the `*_symmetric` arguments. 
