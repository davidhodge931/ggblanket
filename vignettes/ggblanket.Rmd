...
It generally works best to add all aesthetics required to the `gg_*` function. The `gg_*()` function should also be appropriate to be the bottom geom of the plot, given that geoms are drawn in order. 

Subsequently, you may sometimes need to use `gg_blanket()` which defaults to a `blank` geom.

Note directly added aesthetics in the `gg_*` function are globally inherited, and those added via the `mapping` argument are only available locally.  
