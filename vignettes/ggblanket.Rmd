...
* alignment: use conventions aligned with ggplot2 where possible

It is intended to be useful for all levels of experience from beginners all the way to expert ggplot-ers.

## How ggblanket works

To do this, the ggblanket package provides:

1. `gg_*` functions to plot a single geom
2. col and fill aesthetics merged into a single `col` argument
3. Colours customised consistently via a `pal` argument
4. A `facet` argument to facet by a single variable
5. An additional `facet2` argument to facet in a grid 
6. Consistent prefixed arguments to help find arguments to customise
7. Unspecified titles converted to `snakecase::to_sentence`
8. Horizontal labels generally arranged in correct order
9. Nice default scales and theme
10. A `theme` argument for customisation
11. A `gg_theme` function to create a quick theme 
12. A `gg_blank` function for extra flexibility
13. Control over `plotly::ggplotly` tooltips with `add_tooltip_text`
14. Access to other `geom_*` arguments via `...`
15. A way to easily create your own customised plot functions.
...
These functions wrap a `ggplot2::ggplot(aes(...))` function that includes any aesthetics provided with a single `ggplot2::geom_*()` function.
...
### 3. Colours customised consistently via a `pal` argument consistently

These arguments work in the same way regardless of whether a `col` variable is specified or not. This is therefore one less thing for users to remember. 
...
Facetting is treated as if it is an aesthetic, where the users just provide an unquoted variable to facet by. A message is provided to users in the console to remind them of this.
...
If only `facet` is provided, the `gg_*` function under the hood uses `ggplot2::facet_wrap` to wrap by that variable. 

However, if `facet2` is also provided, then the gg_* function uses `ggplot2::facet_grid` to facet in grid of `facet` by `facet2`.
...
In general, arguments follow the prefix with the argument name that they relate to in the `ggplot2` function (e.g. `scale_*_continuous`, `facet_wrap` or `facet_grid`).
...
Note that ggblanket keeps out-of-bound values (i.e. uses scales::oob_keep under the hood). However, you can zoom in using `coord = coord_cartesian(xlim = ..., ylim = ...)`.
...
You can also use `titles = ~.x` to leave unspecified titles as per variable names. 
...
When plots are horizontal, ggblanket ensures y labels and colours are in the right order. 

However, if it gets the legend direction wrong at any point, you can reverse using the `col_legend_rev` argument.
