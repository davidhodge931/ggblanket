...
The `ggplot2::theme_set` function can be used to set the theme globally, such that all subsequent plots will use this theme by default. Once the theme has been set globally, you can override this on a per plot basis using the `theme` argument. 

The `gg_*` function will control what gridlines are present and the placement of the legend. If users want their theme to adjust _everything_ as per the theme, then they can `+` their theme onto the plot instead.

If you wish to set the theme to `ggplot2::theme_grey` with the default `base_size` of 11, then you must it adjust it slightly (e.g. `theme_set(theme_grey(11.01)))`

```{r}
theme_set(theme_grey(11.01))
