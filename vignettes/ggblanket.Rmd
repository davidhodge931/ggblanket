...
Where `mode_orientation = "x"`, the `gg_*` function will remove the y axis line/ticks and the x gridlines from the `mode`. Where it is  `mode_orientation = "y"`, the opposite will occur. If the `gg_*` guesses an incorrect `mode_orientation`, then the user can change this.
...
* colour (i.e. colour/fill used where no colour aesthetic scale)
* annotation colour etc (i.e. for geoms commonly used for annotation)
