...
Note computational speed has been sacrificed to achieve these objectives.
...
15. Use `set_blanket` to adjust the default style
...
Note at the start of every script, the `set_blanket` function should be run to set the style for all subsequent plots. 
...
For plots that that `gg_*` function guesses are not horizontal in orientation, the `gg_*` function will create a symmetric continuous y scale by default with: 

* `y_limits` that are the range of the `y_breaks` 
* `y_expand` of `c(0, 0)`

For the opposite, the vice-versa occurs.
...
The `set_blanket` function is used to set the default style of future plots.

This includes setting the default mode and geom defaults. You can also use `ggplot2::update_geom_defaults` to further fine-tune your geom defaults.

```{r}
set_blanket(
  mode = dark_mode_r(),
  geom_defaults = weave_geom_defaults(col_pal = orange, annotate_pal = darkness[2]),
)
...
p1 <- penguins |>
  ...
  geom_text(aes(x = I(0.25), y = I(0.75), label = "Here"))
...
p2 <- penguins |> 
  ...
  geom_vline(xintercept = 200) +
  geom_text(aes(x = I(0.75), y = I(0.75), label = "Here"))
...
set_blanket(
  mode = light_mode_r(),
  geom_defaults = weave_geom_defaults(plum),
)
