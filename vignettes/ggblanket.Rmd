...
This pretty scale can be turned off easily using `y_limits = c(NA, NA)`, which also reverts the default `y_expand` to `ggplot::waiver()`.
...
### 12. Ability to add multiple `geom\_\*` layers
...
### 14. A powerful gg_blanket function

The ggblanket package contains a powerful `gg_blanket` function that enables the user to specify a geom. This provides additional support for other geoms in extension packages etc.
